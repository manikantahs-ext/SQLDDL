...
Multi-server DDL extraction:
 ...
 - On SQL failure for a database, saves the SQL to a .sql file for inspection and logs diagnostics
...
def main():
    ...
    print("\nPer-server summary:")
    for r in results:
        print(f" - {r['server']}: processed_dbs={r['processed']} failures={len(r['failures'])} output_dir={r.get('output_dir')}")
