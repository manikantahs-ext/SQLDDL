...
-- Database: INHOUSE_MCDCS
-- Exported: 2026-02-05T02:37:52.379054
...
as    
set nocount on    
select * INTO #A1 from accountmcdxcds.dbo.ledger where vdt >=convert(Date,getdate()-10)     
AND CHECKEDBY='ONLINE' AND VTYP=2    
select * INTO #A2 from accountmcdxcds.dbo.ledger where vdt >=convert(Date,getdate()-10)     
AND CHECKEDBY<>'ONLINE' AND VTYP=2    
SELECT B.*,A.DDNO,A.L1_SNO INTO #B1 FROM accountmcdxcds.dbo.LEDGER1 A JOIN #A1 B ON A.VNO=B.VNO     
AND A.VTYP=B.VTYP AND A.LNO=B.LNO    
SELECT B.*,A.DDNO,A.L1_SNO INTO #B2 FROM accountmcdxcds.dbo.LEDGER1 A JOIN #A2 B ON A.VNO=B.VNO     
AND A.VTYP=B.VTYP AND A.LNO=B.LNO    
...
select count(1) as DupCount,Getdate() as ProTime From accountmcdxcds.dbo.ledger where vtyp=2     
...
Begin
Set NoCount On
	Exec accountmcdxcds.dbo.ANGEL_CLIENT_RECEIPT_RECO
Set NoCount Off
End
...
SELECT @MARGINDATE=MAX(MARGINDATE) FROM MCDXCDS.dbo.tbl_clientmargin           
select @sdtcur=sdtcur from ACCOUNTMCDXCDS.dbo.parameter where sdtcur <= GETDATE() and ldtcur >= GETDATE()          
...
BEGIN          
 ...
 MCDXCDS.dbo.tbl_clientmargin WITH (NOLOCK)            
 ...
 from          
 (          
 Select cltcode,vamt,drcr from ACCOUNTMCDXCDS.dbo.ledger a (nolock)           
 ...
 from           
 (select a.Cltcode,a.balance from #bb a join (Select party_code from MCDXCDS.dbo.client2 (nolock)) b  on a.cltcode=b.party_Code ) a            
...
where #Bond_marh.clcode=b.cltcode          
 ...
 (   
  Select cltcode,vamt from ACCOUNTMCDXCDS.dbo.ledger a (nolock)   
...
BEGIN          
 ...
 select @MARGINDATE,a.cltcode,0,0,0,0,0,0,0,a.balance,0,0,0,0,0,0,0,0            
 from           
 (select a.Cltcode,a.balance from #bb1 a join (Select party_code from MCDXCDS.dbo.client2 (nolock)) b  on a.cltcode=b.party_Code ) a            
 left outer join #Bond_marh b           
 on a.cltcode = b.clcode             
 where b.clcode is null            
 /*       
...
select @fromdt=sdtcur from ACCOUNTMCDXCDS.dbo.parameter where sdtnxt = (select sdtcur  from ACCOUNTMCDXCDS.dbo.parameter where sdtcur <= getdate() and ldtcur >=getdate())        
select @todate=ldtcur from ACCOUNTMCDXCDS.dbo.parameter where sdtcur <= getdate() and ldtcur >=getdate()        
-----------END----------------------      
...
BEGIN
	...
	from accountmcdxcds.dbo.ledger b with (nolock)     
	...
	into #led1    
	from accountmcdxcds.dbo.ledger1 with (nolock)    
	...
	*/    
	...
	delete #recodet from #recodet a inner join MCDXCDS.DBO.CLient1 b WITH (NOLOCK) on     
	a.cltcode=b.CL_cODE where a.ddno='0' and (B.cl_type = 'NBF' OR B.cl_type = 'TMF')     
	    
	truncate table BO_client_deposit_recno     
	insert into BO_client_deposit_recno select co_code='MCD',getdate(),* from #recodet (nolock)     
...
BEGIN
	...
	accno=space(10)        
	into #ledger1c        
	from ACCOUNTMCDXCDS.dbo.ledger1 a with (nolock) join #cdet b on a.vno=b.vno and a.vtyp=b.vtyp and a.booktype=b.booktype       
	...
	from ACCOUNTMCDXCDS.dbo.LEDGER a with (nolock) join #cdet b on a.VNO=b.vno and a.booktype=b.booktype and a.vtyp=b.vtyp  
	...
	delete #recodetc from #recodetc a inner join MCDXCDS.dbo.CLient1 b WITH (NOLOCK) on         
	a.cltcode=b.CL_cODE where a.ddno='0' and (B.cl_type = 'NBF' OR B.cl_type = 'TMF')         
	     
	delete from BO_client_deposit_recno where cltcode=@pcode        
	insert into BO_client_deposit_recno select co_code='MCD',getdate(),* from #recodetc (nolock)      
...
/*
declare @fromdt as datetime,@todate as datetime
select @fromdt=sdtcur,@todate=ldtcur from accountmcdxcds.dbo.parameter where sdtcur <= getdate() and ldtcur >=getdate()
*/
...
-- --------------------------------------------------
-- PROCEDURE dbo.Fetch_CliUnreco_02042014_1
-- --------------------------------------------------

CREATE Procedure Fetch_CliUnreco_02042014_1  
as  
 ...
 */      
 ...
 insert into BO_client_deposit_recno_bond select co_code='MCD',getdate(),* from #recodet (nolock)       
...
BEGIN  
 ...
 insert into BO_client_deposit_recno_bond select co_code='MCD',getdate(),* from #recodetc (nolock)        
 ...
 */      
 ...
 insert into BO_client_deposit_recno_GSec select co_code='MCD',getdate(),* from #recodet (nolock)       
...
BEGIN  
 ...
 insert into BO_client_deposit_recno_GSec select co_code='MCD',getdate(),* from #recodetc (nolock)        
...
-- --------------------------------------------------
CREATE procedure [dbo].[MIDWARE_ledger_INHOUSE]  
as                  
                  
set nocount on                  
----- accountMCDXCDS Master                  
          
truncate table INHOUSE_MCDCS.dbo.ledger          
    
/* Fetch Party Ledger */    
insert into INHOUSE_MCDCS.dbo.ledger                
select * from accountMCDXCDS.dbo.ledger with (nolock)     
...
truncate table INHOUSE_MCDCS.dbo.ledger1          
insert into INHOUSE_MCDCS.dbo.ledger1                
select * from accountMCDXCDS.dbo.ledger1 with (nolock)        
...
set nocount on            
----- Account Master            
truncate table INHOUSE_MCDCS.dbo.acmast            
insert into INHOUSE_MCDCS.dbo.acmast             
select * from ACCOUNTMCDXCDS.DBO.acmast with (nolock)     
            
truncate table INHOUSE_MCDCS.dbo.vmast            
insert into INHOUSE_MCDCS.dbo.vmast            
select * from ACCOUNTMCDXCDS.DBO.vmast with (nolock)     
            
truncate table INHOUSE_MCDCS.dbo.parameter             
insert into INHOUSE_MCDCS.dbo.parameter             
select * from ACCOUNTMCDXCDS.DBO.parameter with (nolock)     
          
truncate table INHOUSE_MCDCS.dbo.costmast          
insert into INHOUSE_MCDCS.dbo.costmast             
select * from ACCOUNTMCDXCDS.DBO.costmast with (nolock) --  00:01 secs            
            
            
--- Scrip Master            
truncate table INHOUSE_MCDCS.DBO.FOscrip1            
insert into INHOUSE_MCDCS.DBO.FOscrip1            
select * from MCDXCDS.DBO.FOscrip1 with (nolock)            
            
truncate table INHOUSE_MCDCS.DBO.FOscrip2            
insert into INHOUSE_MCDCS.DBO.FOscrip2            
select * from MCDXCDS.DBO.FOscrip2 with (nolock)            
            
--- Sub-broker            
truncate table INHOUSE_MCDCS.DBO.subbrokers             
insert into INHOUSE_MCDCS.DBO.subbrokers             
select * from MCDXCDS.dbo.subbrokers with (nolock)            
            
--- Branch            
truncate table INHOUSE_MCDCS.DBO.branches            
insert into INHOUSE_MCDCS.DBO.branches            
select * from MCDXCDS.dbo.branches with (nolock)            
            
--- Region            
truncate table INHOUSE_MCDCS.DBO.region            
insert into INHOUSE_MCDCS.DBO.region            
select * from MCDXCDS.dbo.region with (nolock)            
            
--- Brokerage            
truncate table INHOUSE_MCDCS.DBO.Broktable            
insert into INHOUSE_MCDCS.DBO.Broktable            
select * from MCDXCDS.dbo.Broktable with (nolock)            
...
select top 1 @sdate=convert(varchar(13),sauda_DAte) from MCDXCDS.dbo.fosettlement         
--print @sdate        
        
truncate table INHOUSE_MCDCS.dbo.fosettlement 
insert into INHOUSE_MCDCS.dbo.[FOSETTLEMENT]         
select * from MCDXCDS.dbo.fosettlement with (nolock) where sauda_date >= @sdate+' 00:00:00' and  sauda_date <= @sdate+' 23:59:59'                
        
truncate table INHOUSE_MCDCS.dbo.foBILLVALAN 
insert into INHOUSE_MCDCS.dbo.foBILLVALAN        
select * from MCDXCDS.dbo.foBILLVALAN  with (nolock)    where sauda_date >= @sdate+' 00:00:00' and  sauda_date <= @sdate+' 23:59:59'            
    
truncate table  INHOUSE_MCDCS.dbo.tbl_clientmargin 
insert into INHOUSE_MCDCS.dbo.tbl_clientmargin        
select * from MCDXCDS.dbo.tbl_clientmargin  with (nolock)     where margindate >= @sdate+' 00:00:00' and  margindate <= @sdate+' 23:59:59'           
      
truncate table  INHOUSE_MCDCS.dbo.collateraldetails 
insert into INHOUSE_MCDCS.dbo.collateraldetails        
...
Begin      
update INTRANET.CMS.dbo.ncms_batch_process set StartDate=GETDATE() where srno=7  
end  
        
DECLARE @MARGINDATE AS DATETIME,@sdtcur as datetime,@NONCASHconsideration as int = 50          
SELECT @MARGINDATE=MAX(MARGINDATE) FROM MCDXCDS.dbo.tbl_clientmargin  WITH (NOLOCK)          
select @sdtcur=sdtcur from ACCOUNTMCDXCDS.dbo.parameter WITH (NOLOCK) where sdtcur <= GETDATE() and ldtcur >= GETDATE()          
--select @sdtcur=sdtcur from ACCOUNTMCDXCDS.dbo.parameter where ldtcur        =  
--(select ldtprv from ACCOUNTMCDXCDS.dbo.parameter where sdtcur <= GETDATE() and ldtcur >=GETDATE() )  
...
--SELECT @MARGINDATE1=MAX(MARGINDATE) FROM [AngelNseCM].Msajag.dbo.Tbl_Combine_Reporting_peak_Detail with(nolock) WHERE EXCHANGE='MCD' and SEGMENT='FUTURES'  
...
--   and EXCHANGE='MCD' and SEGMENT='FUTURES' and TDAY_MARGIN>0 group by party_code  
--   order by party_code      
...
begin
 ...
 from          
 (          
 Select cltcode,vamt,drcr,VTYP,cdt from ACCOUNTMCDXCDS.dbo.ledger a  WITH (NOLOCK)           
 where VDT >=@sdtcur and VDT <= convert(varchar(11),GETDATE())+' 23:59:59' /*and (VTYP<>35 and isnull(enteredby,'')<>'mtf process') */     
  ...
  /********************************************************************************/  
  and not exists (select cltcode,vdt,vtyp from ACCOUNTMCDXCDS.dbo.ledger b with (nolock)   where VDT between @ccsy and @ccse  
 ...
 */    
 ...
 (   
  Select cltcode,vamt from ACCOUNTMCDXCDS.dbo.ledger a  WITH (NOLOCK)  
  where eDT > convert(varchar(11),GETDATE())+' 23:59:59' and VTYP=15 and DRCR='C'  
 ) b   
 where #NCMS_marh.clcode=b.cltcode   
...
if((CONVERT(VARCHAR(5),GETDATE(),108)>='06:45') and (CONVERT(VARCHAR(5),GETDATE(),108)<='23:59'))      
Begin     
update INTRANET.CMS.dbo.ncms_batch_process set EndDate=GETDATE(),Flag=1 where srno=7  
End      
Set nocount off
...
BEGIN        
 ...
 from        
 (        
 Select cltcode,vamt,drcr from ACCOUNTMCDXCDS.dbo.ledger a  WITH (NOLOCK)         
 where VDT >=@sdtcur and VDT <= convert(varchar(11),GETDATE())+' 23:59:59' /*and (VTYP<>35 and isnull(enteredby,'')<>'mtf process') */   
    /*Added to exclude opening balance of current year*/
  /********************************************************************************/
  and not exists (select cltcode,vdt,vtyp from ACCOUNTMCDXCDS.dbo.ledger b with (nolock)   where VDT between @ccsy and @ccse
...
Begin      
update [196.1.115.132].CMS.dbo.ncms_batch_process set StartDate=GETDATE() where srno=7  
end  
...
if((CONVERT(VARCHAR(5),GETDATE(),108)>='06:59') and (CONVERT(VARCHAR(5),GETDATE(),108)<='23:59'))      
Begin     
update [196.1.115.132].CMS.dbo.ncms_batch_process set EndDate=GETDATE(),Flag=1 where srno=7  
End      
Set nocount off
...
select *,convert(money,0) as c into #NCMS_marh from NCMS_marh where 1=2          
 ...
 namount=ledgeramount+(cash_coll+noncash_coll),            
 FREEcol=ledgeramount+billamount+(cash_coll+noncash_coll-initialmargin-MTMmargin-AddMargin)  from           
 MCDXCDS.dbo.tbl_clientmargin a WITH (NOLOCK)     ,#PO_client p                        
 ...
 from          
 (          
 Select cltcode,vamt,drcr,VTYP,cdt from ACCOUNTMCDXCDS.dbo.ledger a (nolock)   ,#PO_client p              
...
BEGIN        
   ...
   DECLARE @MARGINDATE1 AS DATETIME      
 --set @MARGINDATE1=cast(CAST(getdate()-1 as date)as datetime)
SELECT @MARGINDATE1=MAX(MARGINDATE) FROM [ANAND1].Msajag.dbo.Tbl_Combine_Reporting_peak_Detail with(nolock) WHERE EXCHANGE='MCD' and SEGMENT='FUTURES'
...
BEGIN        
   ...
   DECLARE @MARGINDATE1 AS DATETIME      
 --set @MARGINDATE1=cast(CAST(getdate()-1 as date)as datetime)
SELECT @MARGINDATE1=MAX(MARGINDATE) FROM [196.1.115.196].Msajag.dbo.Tbl_Combine_Reporting_peak_Detail with(nolock) WHERE EXCHANGE='MCD' and SEGMENT='FUTURES'
...
truncate table SEBI_colletral_data   
 ...
 insert into SEBI_colletral_data       
  select *  from msajag.dbo.CollateralDetails WITH (NOLOCK) where exchange='mcd'  and segment='FUTURES'    
   and effdate = (select max(effdate) from msajag.dbo.CollateralDetails WITH (NOLOCK)       
   where effDate <= getdate() and exchange='MCD' and segment='FUTURES')  and Party_Code in (select Party_Code from SEBI_Client)    
...
select @sdtcur=sdtcur/*Sdtnxt*/ from ACCOUNTMCDXCDS.dbo.parameter where ldtcur        =  
(select ldtprv from ACCOUNTMCDXCDS.dbo.parameter where sdtcur <= GETDATE() and ldtcur >=GETDATE()          )  
/********************************************************************************/  
...
--  select party_code,sum(TDAY_MARGIN) as peakMargin into #aa  from [AngelNseCM].Msajag.dbo.Tbl_Combine_Reporting_peak_Detail with(nolock)  
--   where margindate =@MARGINDATE1  
--   and EXCHANGE='MCD' and SEGMENT='FUTURES' and TDAY_MARGIN>0   
...
--end        
 ...
 select @MARGINDATE,a.cltcode,0,0,0,0,0,0,0,a.balance,0,0,0,0,0,0,0,0            
 from           
 (select a.Cltcode,a.balance from #bb a join (Select party_code from MCDXCDS.dbo.client2 (nolock) where party_code in (select party_code from #aaa)) b  on a.cltcode=b.party_Code ) a            
 left outer join #NCMS_marh b           
 on a.cltcode = b.clcode             
 where b.clcode is null       
...
select @sdtcur=Sdtnxt from ACCOUNTMCDXCDS.dbo.parameter where ldtcur        =
(select ldtprv from ACCOUNTMCDXCDS.dbo.parameter where sdtcur <= GETDATE() and ldtcur >=GETDATE()          )
/********************************************************************************/
...
--end        
 ...
 select @MARGINDATE,a.cltcode,0,0,0,0,0,0,0,a.balance,0,0,0,0,0,0,0,0            
 from           
 (select a.Cltcode,a.balance from #bb a join (Select party_code from MCDXCDS.dbo.client2 (nolock)
...
group by cltcode) b            
 ...
 where #NCMS_marh.clcode=b.cltcode   
  ...
  /*added for unsetteledDrbills*/
    select cltcode,sum(VAMT) as UnsettledDrBill into #drbills from ACCOUNTMCDXCDS.dbo.ledger WITH(NOLOCK)  
...
Begin       
   select a.* into #NewcollMCD from msajag.dbo.CollateralDetails a WITH (NOLOCK)     
   where exchange='MCD' and segment='FUTURES' and effdate = (select max(effdate) from msajag.dbo.CollateralDetails WITH (NOLOCK)       
...
CONVERT(MONEY, 0) as Value_AHC      
 into #we  from #NewcollMCD a join      
...
CREATE Proc [dbo].[USP_MTF_Funding](@FilterDate as Datetime) as        
        
set nocount on        
      
DECLARE @MARGINDATE AS DATETIME,@sdtcur as datetime,@NONCASHconsideration as int = 50        
SELECT @MARGINDATE=MAX(MARGINDATE) FROM MCDXCDS.dbo.tbl_clientmargin  WITH (NOLOCK)   where MARGINDATE=@FilterDate
...
AS
    ...
    SELECT vno,
           vtyp
    INTO   #bb
    FROM   accountmcdxcds.dbo.ledger WITH (nolock)
    ...
    FROM   (SELECT a.vdt,
                   a.cltcode,
                   a.vamt
            FROM   accountmcdxcds.dbo.ledger a WITH (nolock)
                   INNER JOIN #bb b
                           ON a.vno = b.vno
                              AND a.vtyp = b.vtyp
...
-- --------------------------------------------------
-- TABLE dbo.bak_rcs_deletion_baddebts_30082012
-- --------------------------------------------------
CREATE TABLE [dbo].[bak_rcs_deletion_baddebts_30082012]
(
...
(
    ...
    [C_U_CAL] INT NULL,
    [PriceNumerator] NUMERIC(18, 4) NULL,
    [PriceDenominator] NUMERIC(18, 4) NULL,
    [GeneralNumerator] NUMERIC(18, 4) NULL,
    [GeneralDenominator] NUMERIC(18, 4) NULL,
    [LotNumerator] NUMERIC(18, 4) NULL,
    [LotDenominator] NUMERIC(18, 4) NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.parameter_04022011
-- --------------------------------------------------
CREATE TABLE [dbo].[parameter_04022011]
(
    [Sdtcur] DATETIME NULL,
    [Ldtcur] DATETIME NULL,
    [Ldtprv] DATETIME NULL,
...
CREATE View accbill  
as  
--select * from [196.1.115.236].mcdxcds.dbo.foaccbill where start_date <='Jan 31  2011 23:59:59'   
--UNION   
--select * from mcdxcds.dbo.foaccbill where start_date >='Feb  1 2011 00:00:00'  
select * from mcdxcds.dbo.foaccbill where start_date >='Apr  1 2011 00:00:00'
...
-- --------------------------------------------------
create view acccategory as select * from accountmcdxcds.dbo.acccategory with (nolock)
...
-- --------------------------------------------------
create view accountscategory as select * from accountmcdxcds.dbo.accountscategory with (nolock)
...
-- VIEW dbo.acmast
-- --------------------------------------------------
create View acmast as select * from accountmcdxcds.dbo.acmast (Nolock)
...
from mcdxcds.dbo.tblpradnyausers a with (nolock) left outer join mcdxcds.dbo.tblUserControlMaster b with (nolock)  
on a.fldauto=b.flduserid  
where fldadminauto in (8,11,10,6,1)
...
-- --------------------------------------------------
create view angel_BANK_Accno as select * from accountmcdxcds.dbo.angel_BANK_Accno with (nolock)
...
-- --------------------------------------------------
create view Angel_client_deposit_recno as select * from accountmcdxcds.dbo.Angel_client_deposit_recno with (nolock)
...
-- --------------------------------------------------
create view angel_temp_mis as select * from accountmcdxcds.dbo.angel_temp_mis with (nolock)
...
-- VIEW dbo.baddebt
-- --------------------------------------------------
create view baddebt as select * from accountmcdxcds.dbo.baddebt with (nolock)
...
-- VIEW dbo.BAK_acmast
-- --------------------------------------------------
create view BAK_acmast as select * from accountmcdxcds.dbo.BAK_acmast with (nolock)
...
-- --------------------------------------------------
create view BANKDETAILS_FINAL as select * from accountmcdxcds.dbo.BANKDETAILS_FINAL with (nolock)
...
-- VIEW dbo.Banklocat
-- --------------------------------------------------
create view Banklocat as select * from accountmcdxcds.dbo.Banklocat with (nolock)
...
-- VIEW dbo.Bankreco
-- --------------------------------------------------
create view Bankreco as select * from accountmcdxcds.dbo.Bankreco with (nolock)
...
-- VIEW dbo.BILLLOG
-- --------------------------------------------------
create view BILLLOG as select * from accountmcdxcds.dbo.BILLLOG with (nolock)
...
-- VIEW dbo.Billmatch
-- --------------------------------------------------
create view Billmatch as select * from accountmcdxcds.dbo.Billmatch with (nolock)
...
-- VIEW dbo.BillPosted
-- --------------------------------------------------
create View BillPosted as select * from accountmcdxcds.dbo.BillPosted (Nolock)
...
-- --------------------------------------------------
create view BILLPROCESS as select * from accountmcdxcds.dbo.BILLPROCESS with (nolock)
...
-- VIEW dbo.Booktype
-- --------------------------------------------------
create view Booktype as select * from accountmcdxcds.dbo.Booktype with (nolock)
...
-- --------------------------------------------------
create view Branchaccounts as select * from accountmcdxcds.dbo.Branchaccounts with (nolock)

GO

-- --------------------------------------------------
-- VIEW dbo.branchaccounts_new
-- --------------------------------------------------
create view branchaccounts_new as select * from accountmcdxcds.dbo.branchaccounts_new with (nolock)
...
-- --------------------------------------------------
create view branchwiseclbal as select * from accountmcdxcds.dbo.branchwiseclbal with (nolock)
...
-- VIEW dbo.bseled
-- --------------------------------------------------
create view bseled as select * from accountmcdxcds.dbo.bseled with (nolock)
...
-- VIEW dbo.Category
-- --------------------------------------------------
create view Category as select * from accountmcdxcds.dbo.Category with (nolock)
...
-- VIEW dbo.catmast
-- --------------------------------------------------
create view catmast as select * from accountmcdxcds.dbo.catmast with (nolock)
...
-- --------------------------------------------------
create view chequeformats as select * from accountmcdxcds.dbo.chequeformats with (nolock)
...
-- --------------------------------------------------
create view Class_SQLTableIndex as select * from accountmcdxcds.dbo.Class_SQLTableIndex with (nolock)
...
-- --------------------------------------------------
create view Class_SQLTableListing as select * from accountmcdxcds.dbo.Class_SQLTableListing with (nolock)

GO

-- --------------------------------------------------
-- VIEW dbo.costmast_bak
-- --------------------------------------------------
create view costmast_bak as select * from accountmcdxcds.dbo.costmast_bak with (nolock)

GO

-- --------------------------------------------------
-- VIEW dbo.Costmast_KIRAN
-- --------------------------------------------------
create view Costmast_KIRAN as select * from accountmcdxcds.dbo.Costmast_KIRAN with (nolock)

GO

-- --------------------------------------------------
-- VIEW dbo.costmast_new
-- --------------------------------------------------
create view costmast_new as select * from accountmcdxcds.dbo.costmast_new with (nolock)
...
-- VIEW dbo.creditors
-- --------------------------------------------------
create view creditors as select * from accountmcdxcds.dbo.creditors with (nolock)
...
-- VIEW dbo.ddvno
-- --------------------------------------------------
create view ddvno as select * from accountmcdxcds.dbo.ddvno with (nolock)
...
-- --------------------------------------------------
create view dtproperties as select * from accountmcdxcds.dbo.dtproperties with (nolock)
...
-- VIEW dbo.edtclbal
-- --------------------------------------------------
create view edtclbal as select * from accountmcdxcds.dbo.edtclbal with (nolock)
...
-- --------------------------------------------------
create view Errvouchers as select * from accountmcdxcds.dbo.Errvouchers with (nolock)
...
-- VIEW dbo.grpmast
-- --------------------------------------------------
create View grpmast as select * from accountmcdxcds.dbo.grpmast (Nolock)

GO

-- --------------------------------------------------
-- VIEW dbo.Inhouse_MCDCS_Margin
-- --------------------------------------------------
Create View Inhouse_MCDCS_Margin
as
select party_code,margindate,cash_coll+noncash_coll as coll, initialmargin+mtmmargin+addmargin as margin from TBL_CLIENTMARGIN with (nolock)
...
-- VIEW dbo.KIRAN1
-- --------------------------------------------------
create view KIRAN1 as select * from accountmcdxcds.dbo.KIRAN1 with (nolock)
...
-- --------------------------------------------------
create view Lastchequeno as select * from accountmcdxcds.dbo.Lastchequeno with (nolock)
...
-- VIEW dbo.Lastvno
-- --------------------------------------------------
create view Lastvno as select * from accountmcdxcds.dbo.Lastvno with (nolock)
...
/*
select * from      
(select * from [196.1.115.236].accountmcdxcds.dbo.ledger with (nolock) where vdt >='Apr 1 2010 00:00:00' and vdt <='Jan 31 2011 23:59:59') a      
union all      
*/
...
/* where vdt >= 'Feb 1 2011 00:00:00' and not (vdt = 'Feb 1 2011 00:00:00' and vtyp = 18) */
...
(select * from [196.1.115.236].accountmcdxcds.dbo.ledger with (nolock) where vdt >='Apr  1 2010 00:00:00' and vdt <='Jan 31  2011 23:59:59') a      
union all      
select * from accountmcdxcds.dbo.ledger with (nolock) where 
vdt >= 'Feb  1 2011 00:00:00'    
...
as  
select * from  
(select * from [196.1.115.236].accountmcdxcds.dbo.ledger with (nolock) where vdt >='Apr  1 2010 00:00:00') a  
union all  
select * from accountmcdxcds.dbo.ledger where vdt > (  
select convert(datetime,convert(varchar(11),max(vdt)))+1 as vdt from [196.1.115.236].accountmcdxcds.dbo.ledger with (nolock)  
) and vtyp <> 18
...
-- --------------------------------------------------
create view LEDGER_BRANCHBREAKUP as select * from accountmcdxcds.dbo.LEDGER_BRANCHBREAKUP with (nolock)
...
-- VIEW dbo.ledger_log
-- --------------------------------------------------
create view ledger_log as select * from accountmcdxcds.dbo.ledger_log with (nolock)
...
-- VIEW dbo.ledger_pay
-- --------------------------------------------------
create view ledger_pay as select * from accountmcdxcds.dbo.ledger_pay with (nolock)
...
-- --------------------------------------------------
create view Ledger_Report as select * from accountmcdxcds.dbo.Ledger_Report with (nolock)
...
-- --------------------------------------------------
create view Ledger_Temp_New as select * from accountmcdxcds.dbo.Ledger_Temp_New with (nolock)
...
-- --------------------------------------------------
create view LEDGER_TEMP_NEW_PARTYLEDGER as select * from accountmcdxcds.dbo.LEDGER_TEMP_NEW_PARTYLEDGER with (nolock)
...
-- --------------------------------------------------
create view LEDGER_TRIG as select * from accountmcdxcds.dbo.LEDGER_TRIG with (nolock)
...
/*
SELECT * FROM [196.1.115.236].accountmcdxcds.dbo.ledger1 with (nolock) WHERE VNO <= '201101319999'        
union all        
SELECT * FROM accountmcdxcds.dbo.ledger1 with (nolock) WHERE VNO > '201102010000' or VNO like  '201100%'    
--SELECT * FROM accountmcdxcds.dbo.ledger1 with (nolock) WHERE VNO > '201104010000' or VNO like  '201100%' 
*/
...
-- --------------------------------------------------
create view Ledger1_log as select * from accountmcdxcds.dbo.Ledger1_log with (nolock)
...
-- --------------------------------------------------
create view LEDGER1_MANUALRECO_LOG as select * from accountmcdxcds.dbo.LEDGER1_MANUALRECO_LOG with (nolock)
...
-- --------------------------------------------------
create view ledger1_pay as select * from accountmcdxcds.dbo.ledger1_pay with (nolock)
...
-- --------------------------------------------------
create view LEDGER1_TRIG as select * from accountmcdxcds.dbo.LEDGER1_TRIG with (nolock)
...
-- VIEW dbo.Ledger2
-- --------------------------------------------------
create view Ledger2 as select * from accountmcdxcds.dbo.Ledger2 with (nolock)
...
-- --------------------------------------------------
create view ledger2_log as select * from accountmcdxcds.dbo.ledger2_log with (nolock)
...
-- VIEW dbo.ledger3
-- --------------------------------------------------
create view ledger3 as select * from accountmcdxcds.dbo.ledger3 with (nolock)
...
-- --------------------------------------------------
create view ledger3_log as select * from accountmcdxcds.dbo.ledger3_log with (nolock)
...
-- --------------------------------------------------
create view Ledgerbalances as select * from accountmcdxcds.dbo.Ledgerbalances with (nolock)
...
-- --------------------------------------------------
create view LEDGERRENUM_LOG as select * from accountmcdxcds.dbo.LEDGERRENUM_LOG with (nolock)
...
as  
--select * from  
--(select * from [196.1.115.236].accountmcdxcds.dbo.marginledger with (nolock) where vdt >='Apr  1 2010 00:00:00') a  
--union all  
--select * from accountmcdxcds.dbo.marginledger where vdt > (  
--select convert(datetime,convert(varchar(11),max(vdt)))+1 as vdt from [196.1.115.236].accountmcdxcds.dbo.marginledger with (nolock)  
--) and vtyp <> 18  

select * from accountmcdxcds.dbo.marginledger where vdt > ='Apr  1 2011 00:00:00'
...
-- --------------------------------------------------
create view marginledger_log as select * from accountmcdxcds.dbo.marginledger_log with (nolock)
...
-- --------------------------------------------------
create view Matchdetails as select * from accountmcdxcds.dbo.Matchdetails with (nolock)
...
-- --------------------------------------------------
create view matchdetails_log as select * from accountmcdxcds.dbo.matchdetails_log with (nolock)

GO

-- --------------------------------------------------
-- VIEW dbo.Mis_Temp_ACPLMCD
-- --------------------------------------------------
create view Mis_Temp_ACPLMCD as select * from accountmcdxcds.dbo.Mis_Temp_ACPLMCD with (nolock)
...
-- --------------------------------------------------
create view MULTIBANKID as select * from accountmcdxcds.dbo.MULTIBANKID with (nolock)
...
as
	...
	( 
		 ...
		 from msajag.dbo.CollateralDetails WITH (NOLOCK) where exchange='mcd'  and segment='FUTURES'
   and effdate = (select max(effdate) from msajag.dbo.CollateralDetails WITH (NOLOCK)   
   where effDate <= getdate() and exchange='MCD' and segment='FUTURES') 
...
-- --------------------------------------------------
create view NEWPARTYLEDGER as select * from accountmcdxcds.dbo.NEWPARTYLEDGER with (nolock)
...
-- --------------------------------------------------
create view opencltcode as select * from accountmcdxcds.dbo.opencltcode with (nolock)
...
-- VIEW dbo.Owner
-- --------------------------------------------------
create view Owner as select * from accountmcdxcds.dbo.Owner with (nolock)
...
-- VIEW dbo.p1
-- --------------------------------------------------
create view p1 as select * from accountmcdxcds.dbo.p1 with (nolock)
...
-- VIEW dbo.Payadv
-- --------------------------------------------------
create view Payadv as select * from accountmcdxcds.dbo.Payadv with (nolock)
...
-- --------------------------------------------------
create view PostBillLedger as select * from accountmcdxcds.dbo.PostBillLedger with (nolock)
...
-- --------------------------------------------------
create view POSTBILLLEDGER2 as select * from accountmcdxcds.dbo.POSTBILLLEDGER2 with (nolock)
...
-- --------------------------------------------------
create view RecPay_Table as select * from accountmcdxcds.dbo.RecPay_Table with (nolock)
...
-- --------------------------------------------------
create view RecPay_Table_Tmp as select * from accountmcdxcds.dbo.RecPay_Table_Tmp with (nolock)
...
-- VIEW dbo.sachin1
-- --------------------------------------------------
create view sachin1 as select * from accountmcdxcds.dbo.sachin1 with (nolock)
...
-- --------------------------------------------------
create view Selection_Levels_PartyLedger as select * from accountmcdxcds.dbo.Selection_Levels_PartyLedger with (nolock)
...
-- --------------------------------------------------
create view SELECTION_QUERIES_TABLE as select * from accountmcdxcds.dbo.SELECTION_QUERIES_TABLE with (nolock)
...
-- --------------------------------------------------
create view sysdiagrams as select * from accountmcdxcds.dbo.sysdiagrams with (nolock)
...
-- --------------------------------------------------
create view Tbl_GLReport as select * from accountmcdxcds.dbo.Tbl_GLReport with (nolock)
...
-- --------------------------------------------------
create view Tbl_GLReport_New as select * from accountmcdxcds.dbo.Tbl_GLReport_New with (nolock)
...
-- --------------------------------------------------
create view TBL_MASTER_PARTYLEDGER as select * from accountmcdxcds.dbo.TBL_MASTER_PARTYLEDGER with (nolock)
...
-- --------------------------------------------------
create view Tbl_OppBalance as select * from accountmcdxcds.dbo.Tbl_OppBalance with (nolock)
...
-- VIEW dbo.TEMP_FILE2
-- --------------------------------------------------
create view TEMP_FILE2 as select * from accountmcdxcds.dbo.TEMP_FILE2 with (nolock)
...
-- VIEW dbo.temp_mcd
-- --------------------------------------------------
create view temp_mcd as select * from accountmcdxcds.dbo.temp_mcd with (nolock)
...
-- --------------------------------------------------
create view temp_notin_multi1 as select * from accountmcdxcds.dbo.temp_notin_multi1 with (nolock)
...
-- --------------------------------------------------
create view Tempbilldetails as select * from accountmcdxcds.dbo.Tempbilldetails with (nolock)
...
-- --------------------------------------------------
create view Templedger2 as select * from accountmcdxcds.dbo.Templedger2 with (nolock)
...
-- --------------------------------------------------
create view Temppartybalances1 as select * from accountmcdxcds.dbo.Temppartybalances1 with (nolock)
...
-- VIEW dbo.USERRIGHTS
-- --------------------------------------------------
create view USERRIGHTS as select * from accountmcdxcds.dbo.USERRIGHTS with (nolock)
...
-- --------------------------------------------------
create view userrights_bak as select * from accountmcdxcds.dbo.userrights_bak with (nolock)
...
-- --------------------------------------------------
create view Userwritestable as select * from accountmcdxcds.dbo.Userwritestable with (nolock)
...
-- VIEW dbo.V2_LastVno
-- --------------------------------------------------
create view V2_LastVno as select * from accountmcdxcds.dbo.V2_LastVno with (nolock)
...
-- --------------------------------------------------
create view V2_OFFLINE_CC_ENTRIES as select * from accountmcdxcds.dbo.V2_OFFLINE_CC_ENTRIES with (nolock)
...
-- --------------------------------------------------
create view V2_UPLOADED_FILES as select * from accountmcdxcds.dbo.V2_UPLOADED_FILES with (nolock)
...
-- VIEW dbo.VMAIN
-- --------------------------------------------------
create view VMAIN as select * from accountmcdxcds.dbo.VMAIN with (nolock)
...
-- VIEW dbo.vmain_bak
-- --------------------------------------------------
create view vmain_bak as select * from accountmcdxcds.dbo.vmain_bak with (nolock)
...
-- VIEW dbo.VMAST
-- --------------------------------------------------
create View VMAST as select * from accountmcdxcds.dbo.VMAST (Nolock)
