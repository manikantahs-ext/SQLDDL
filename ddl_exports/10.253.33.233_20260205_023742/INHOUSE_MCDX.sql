...
-- Database: INHOUSE_MCDX
-- Exported: 2026-02-05T02:37:52.895895
...
-- --------------------------------------------------
-- INDEX dbo.prorderwisedetails_FO_RTB
-- --------------------------------------------------
CREATE CLUSTERED INDEX [IX_Sauda_date] ON [dbo].[prorderwisedetails_FO_RTB] ([Sauda_Date], [Party_Code])
...
-- --------------------------------------------------
CREATE NONCLUSTERED INDEX [NonClusteredIndex_Partycode] ON [dbo].[SEBI_Client] ([party_code])
...
as    
set nocount on    
select * INTO #A1 from ACCOUNTMCDX.dbo.ledger where vdt >=convert(Date,getdate()-10) AND CHECKEDBY='ONLINE' AND VTYP=2    
select * INTO #A2 from ACCOUNTMCDX.dbo.ledger where vdt >=convert(Date,getdate()-10) AND CHECKEDBY<>'ONLINE' AND VTYP=2    
SELECT B.*,A.DDNO,A.L1_SNO INTO #B1 FROM ACCOUNTMCDX.dbo.LEDGER1 A JOIN #A1 B ON A.VNO=B.VNO     
AND A.VTYP=B.VTYP AND A.LNO=B.LNO    
SELECT B.*,A.DDNO,A.L1_SNO INTO #B2 FROM ACCOUNTMCDX.dbo.LEDGER1 A JOIN #A2 B ON A.VNO=B.VNO     
AND A.VTYP=B.VTYP AND A.LNO=B.LNO    
...
select count(1) as DupCount,Getdate() as ProTime From ACCOUNTMCDX.dbo.ledger where vtyp=2     
...
SELECT @MARGINDATE=MAX(MARGINDATE) FROM MCDX.dbo.tbl_clientmargin WITH (NOLOCK)                   
select @sdtcur=sdtcur from ACCOUNTMCDX.dbo.parameter WITH (NOLOCK) where sdtcur <= GETDATE() and ldtcur >= GETDATE()                  
--select @sdtcur=sdtcur from ACCOUNTMCDX.dbo.parameter where ldtcur        =  
--(select ldtprv from ACCOUNTMCDX.dbo.parameter where sdtcur <= GETDATE() and ldtcur >=GETDATE())                  
...
CREATE CLUSTERED INDEX IDXCL ON #bond_marh(CLCODE)    

  select distinct client_code into #PO_client from  [196.1.115.132].bond.dbo.tbl_BONDOrderEntry with(nolock) where  order_status='Pending'   
      create index #PO_cli on #PO_client(client_code)  
...
begin  
  ...
  select party_code,peakMargin*@peakVar as peakMargin into #peak from #aa  
   ...
   into #data from           
   MCDX.dbo.tbl_clientmargin WITH (NOLOCK)            
  ...
  GROUP BY party_code       
   ...
   into #main from           
   MCDX.dbo.tbl_clientmargin a WITH (NOLOCK) ,#PO_client p                       
 where margindate = @MARGINDATE   and a.party_code=p.client_code   
...
/*********************************************/          
 ...
 from                  
 (                  
 Select cltcode,vamt,drcr from ACCOUNTMCDX.dbo.ledger a (nolock)  ,#PO_client p                         
 where VDT >=@sdtcur and VDT <= convert(varchar(11),GETDATE())+' 23:59:59'/* and (VTYP<>35 and isnull(enteredby,'')<>'mtf process') */    
  and CLTCODE=p.client_code   
    /*Added to exclude opening balance of current year*/  
  /********************************************************************************/  
  and not exists (select cltcode,vdt,vtyp from ACCOUNTMCDX.dbo.ledger b with (nolock)   where VDT between @ccsy and @ccse  
 ...
 from                   
 (select a.Cltcode,a.balance from #bb a join (Select party_code from MCDX.dbo.client2 (nolock)) b  on a.cltcode=b.party_Code ) a                    
 left outer join #bond_marh b                   
 on a.cltcode = b.clcode                     
 where b.clcode is null                    
      
 select * into #bond_Colleteral from NCMS_Colleteral      
                 
 insert into #bond_marh(Sauda_Date,Clcode,imargin,span,total,mtm,received,shortage,net,ledgeramount,cash_coll,non_cash,exposure,mtm_loss,deposit,net_exposure,UnrecoCr,OtherDr)                      
 select @MARGINDATE,a.party_code,0,0,0,0,0,0,0,0,a.CashColl,a.NonCashColl,0,0,0,0,0,0                    
 from #bond_Colleteral a with (nolock) left outer join #bond_marh b on a.party_code = b.clcode where b.clcode is null                   
                
 update #bond_marh set ledgeramount=balance from #bb b  where #bond_marh.clcode=b.cltcode                    
 update #bond_marh set cash_coll=CashColl,non_Cash=NonCashColl from #bond_Colleteral b with (nolock)         
 where #bond_marh.clcode=b.party_Code                    
...
(select substring(Cltcode,3,10) as Cltcode,balance as VBal from #bb where cltcode like '98%' and Balance < 0) b                    
 ...
 (select cltcode,SUM(Cramt) as UnRecoCr from BO_client_deposit_recno_bond group by cltcode) b                    
 where #bond_marh.clcode=b.cltcode                   
 ...
 (   
  Select cltcode,vamt from ACCOUNTMCDX.dbo.ledger a (nolock)   
...
BEGIN                  
 ...
 from                  
 (                  
  Select cltcode,vamt,drcr from ACCOUNTMCDX.dbo.ledger a (nolock)                   
 ...
 select @MARGINDATE,a.cltcode,0,0,0,0,0,0,0,a.balance,0,0,0,0,0,0,0,0                    
 from                   
 (select a.Cltcode,a.balance from #bb1 a join (Select party_code from MCDX.dbo.client2 (nolock)) b  on a.cltcode=b.party_Code ) a                    
 left outer join #bond_marh b                   
 on a.cltcode = b.clcode                     
 where b.clcode is null                    
...
select @fromdt=sdtcur from ACCOUNTMCDX.dbo.parameter where sdtnxt = (select sdtcur  from ACCOUNTMCDX.dbo.parameter where sdtcur <= getdate() and ldtcur >=getdate())        
select @todate=ldtcur from ACCOUNTMCDX.dbo.parameter where sdtcur <= getdate() and ldtcur >=getdate()        
-----------END----------------------      
...
BEGIN
	...
	from accountmcdx.dbo.ledger b with (nolock)       
	...
	clear_mode,L1_SNo      
	into #led1      
	from accountmcdx.dbo.ledger1 with (nolock)      
	...
	into #recodet       
	From accountmcdx.dbo.LEDGER l with (nolock)      
	...
	*/      
	...
	delete #recodet from #recodet a inner join MCDX.DBO.CLient1 b WITH (NOLOCK) on       
...
BEGIN
	...
	accno=space(10)        
	into #ledger1c        
	from ACCOUNTMCDX.dbo.ledger1 a with (nolock) join #cdet b on a.vno=b.vno and a.vtyp=b.vtyp and a.booktype=b.booktype       
	...
	from ACCOUNTMCDX.dbo.LEDGER a with (nolock) join #cdet b on a.VNO=b.vno and a.booktype=b.booktype and a.vtyp=b.vtyp  
	...
	delete #recodetc from #recodetc a inner join MCDX.dbo.CLient1 b WITH (NOLOCK) on         
	a.cltcode=b.CL_cODE where a.ddno='0' and (B.cl_type = 'NBF' OR B.cl_type = 'TMF')         
...
declare @fromdt as datetime,@todate as datetime  
select @fromdt=sdtcur,@todate=ldtcur from accountmcdx.dbo.parameter where sdtcur <= getdate() and ldtcur >=getdate()  
...
truncate table BO_client_deposit_recno   
insert into BO_client_deposit_recno select co_code='MCX',getdate(),* from #recodet (nolock)   
...
BEGIN
	...
	insert into BO_client_deposit_recno_Bond select co_code='MCX',getdate(),* from #recodetc (nolock)      
...
BEGIN
  ...
  create index #PO_cli on #PO_client_unreco(Party_code)  
	...
	select b.vtyp, b.booktype, b.vno, b.lno,accno=b.cltcode into #vdet      
	from accountmcdx.dbo.ledger b with (nolock)       ,#PO_client_unreco p              
 where vdt >=@fromdt and vdt <=@todate and (vtyp=2 or vtyp=3)    and b.CLTCODE=p.party_code        
	...
	clear_mode,L1_SNo      
	into #led1      
	from accountmcdx.dbo.ledger1 t with (nolock), #vdet l where l.vtyp = t.vtyp and l.vno= t.vno and l.booktype = t.booktype    
...
truncate table INHOUSE_MCDX.dbo.ledger        
  
/* Fetch Party Ledger */  
insert into INHOUSE_MCDX.dbo.ledger              
select * from accountmcdx.dbo.ledger with (nolock)   
...
truncate table INHOUSE_MCDX.dbo.ledger1        
insert into INHOUSE_MCDX.dbo.ledger1              
select * from accountmcdx.dbo.ledger1 with (nolock)      
...
set nocount on          
----- Account Master          
truncate table INHOUSE_MCDX.dbo.acmast          
insert into INHOUSE_MCDX.dbo.acmast           
select * from ACCOUNTmcdx.DBO.acmast with (nolock)   
          
truncate table INHOUSE_MCDX.dbo.vmast          
insert into INHOUSE_MCDX.dbo.vmast          
select * from ACCOUNTmcdx.DBO.vmast with (nolock)   
          
truncate table INHOUSE_MCDX.dbo.parameter           
insert into INHOUSE_MCDX.dbo.parameter           
select * from ACCOUNTmcdx.DBO.parameter with (nolock)   
        
truncate table INHOUSE_MCDX.dbo.costmast        
insert into INHOUSE_MCDX.dbo.costmast           
select * from ACCOUNTmcdx.DBO.costmast with (nolock) --  00:01 secs          
          
          
--- Scrip Master          
truncate table INHOUSE_MCDX.DBO.FOscrip1          
insert into INHOUSE_MCDX.DBO.FOscrip1          
select * from MCDX.DBO.FOscrip1 with (nolock)          
          
truncate table INHOUSE_MCDX.DBO.FOscrip2          
insert into INHOUSE_MCDX.DBO.FOscrip2          
select * from MCDX.DBO.FOscrip2 with (nolock)          
          
--- Sub-broker          
truncate table INHOUSE_MCDX.DBO.subbrokers           
insert into INHOUSE_MCDX.DBO.subbrokers           
select * from MCDX.dbo.subbrokers with (nolock)          
          
--- Branch          
truncate table INHOUSE_MCDX.DBO.branches          
insert into INHOUSE_MCDX.DBO.branches          
select * from MCDX.dbo.branches with (nolock)          
          
--- Region          
truncate table INHOUSE_MCDX.DBO.region          
insert into INHOUSE_MCDX.DBO.region          
select * from MCDX.dbo.region with (nolock)          
          
--- Brokerage          
truncate table INHOUSE_MCDX.DBO.Broktable          
insert into INHOUSE_MCDX.DBO.Broktable          
select * from MCDX.dbo.Broktable with (nolock)          
...
select top 1 @sdate=convert(varchar(13),sauda_DAte) from MCDX.dbo.fosettlement       
--print @sdate      
      
truncate table  INHOUSE_MCDX.dbo.fosettlement 
insert into INHOUSE_MCDX.dbo.fosettlement       
select * from MCDX.dbo.fosettlement with (nolock)  where sauda_date >= @sdate+' 00:00:00' and  sauda_date <= @sdate+' 23:59:59'           
      
truncate table INHOUSE_MCDX.dbo.foBILLVALAN 
insert into INHOUSE_MCDX.dbo.foBILLVALAN      
select * from MCDX.dbo.foBILLVALAN  with (nolock) where sauda_date >= @sdate+' 00:00:00' and  sauda_date <= @sdate+' 23:59:59'        
  
truncate table INHOUSE_MCDX.dbo.tbl_clientmargin 
insert into INHOUSE_MCDX.dbo.tbl_clientmargin      
select * from MCDX.dbo.tbl_clientmargin  with (nolock) where margindate >= @sdate+' 00:00:00' and  margindate <= @sdate+' 23:59:59'           
    
truncate table  INHOUSE_MCDX.dbo.collateraldetails 
insert into INHOUSE_MCDX.dbo.collateraldetails      
...
Begin              
update INTRANET.CMS.dbo.ncms_batch_process set StartDate=GETDATE() where srno=12  
end                
DECLARE @MARGINDATE AS DATETIME,@sdtcur as datetime,@NONCASHconsideration as int = 50                  
SELECT @MARGINDATE=MAX(MARGINDATE) FROM MCDX.dbo.tbl_clientmargin WITH (NOLOCK)                   
select @sdtcur=sdtcur from ACCOUNTMCDX.dbo.parameter WITH (NOLOCK) where sdtcur <= GETDATE() and ldtcur >= GETDATE()                  
...
select @sdtcur=sdtcur/*sdtnxt*/ from ACCOUNTMCDX.dbo.parameter where ldtcur        =  
(select ldtprv from ACCOUNTMCDX.dbo.parameter where sdtcur <= GETDATE() and ldtcur >=GETDATE()          )  
/********************************************************************************/  
...
begin
 ...
 from                  
 (                  
 Select cltcode,vamt,drcr,CDT,VTYP from ACCOUNTMCDX.dbo.ledger a (nolock)   ,#PO_client p                        
...
exec BSEFO.dbo.NCMS_POdet
if((CONVERT(VARCHAR(5),GETDATE(),108)>='06:45') and (CONVERT(VARCHAR(5),GETDATE(),108)<='23:59'))      
Begin    
update INTRANET.CMS.dbo.ncms_batch_process set EndDate=GETDATE(),Flag=1 where srno=12               
end  
Set nocount off
...
DECLARE @MARGINDATE AS DATETIME,@sdtcur as datetime,@NONCASHconsideration as int = 50                
SELECT @MARGINDATE=MAX(MARGINDATE) FROM MCDX.dbo.tbl_clientmargin                 
select @sdtcur=sdtcur from ACCOUNTMCDX.dbo.parameter where sdtcur <= GETDATE() and ldtcur >= GETDATE()                
...
Begin            
update [196.1.115.132].CMS.dbo.ncms_batch_process set StartDate=GETDATE() where srno=12
end              
DECLARE @MARGINDATE AS DATETIME,@sdtcur as datetime,@NONCASHconsideration as int = 50                
SELECT @MARGINDATE=MAX(MARGINDATE) FROM MCDX.dbo.tbl_clientmargin WITH (NOLOCK)          
...
DECLARE @MARGINDATE1 AS DATETIME      
-- set @MARGINDATE1=cast(CAST(getdate()-1 as date)as datetime)
SELECT @MARGINDATE1=MAX(MARGINDATE) FROM [196.1.115.196].Msajag.dbo.Tbl_Combine_Reporting_peak_Detail with(nolock) WHERE EXCHANGE='MCX' and SEGMENT='FUTURES'
...
/*********************************************/        
 ...
 from                
 (                
 Select cltcode,vamt,drcr,CDT,VTYP from ACCOUNTMCDX.dbo.ledger a (nolock)                 
 where VDT >=@sdtcur and VDT <= convert(varchar(11),GETDATE())+' 23:59:59'/* and (VTYP<>35 and isnull(enteredby,'')<>'mtf process') */ 
    /*Added to exclude opening balance of current year*/
  /********************************************************************************/
  and not exists (select cltcode,vdt,vtyp from ACCOUNTMCDX.dbo.ledger b with (nolock)   where VDT between @ccsy and @ccse
...
if((CONVERT(VARCHAR(5),GETDATE(),108)>='06:59') and (CONVERT(VARCHAR(5),GETDATE(),108)<='23:59'))    
Begin  
update [196.1.115.132].CMS.dbo.ncms_batch_process set EndDate=GETDATE(),Flag=1 where srno=12             
end
Set nocount off
...
select @sdtcur=sdtnxt/*sdtcur*/ from ACCOUNTMCDX.dbo.parameter where ldtcur        =  
(select ldtprv from ACCOUNTMCDX.dbo.parameter where sdtcur <= GETDATE() and ldtcur >=GETDATE()          )  
/********************************************************************************/  
...
CREATE CLUSTERED INDEX IDXCL ON #NRMS_marh(CLCODE)  
IF @pcode is null                
BEGIN                
 ...
 update #NRMS_marh set cash_coll=CashColl,non_Cash=NonCashColl from NCMS_Colleteral b with (nolock)       
 where #NRMS_marh.clcode=b.party_Code                  
...
truncate table SEBI_colletral_data   
 ...
 select *  from anand1.msajag.dbo.CollateralDetails c WITH (NOLOCK)     
   where exchange='MCX' and segment='FUTURES' and effdate = (select max(effdate) from anand1.msajag.dbo.CollateralDetails WITH (NOLOCK)     
   where effDate >= getdate()-10 and effDate <= getdate() and exchange='MCX' and segment='FUTURES')   and 
   exists  (select Party_Code from SEBI_Client s with(nolock) where s.party_code=c.Party_Code)    
end
...
--  select party_code,sum(TDAY_MARGIN) as peakMargin into #aa  from [AngelNseCM].Msajag.dbo.Tbl_Combine_Reporting_peak_Detail with(nolock)  
--   where margindate =@MARGINDATE1  
--   and EXCHANGE='MCX' and SEGMENT='FUTURES' and TDAY_MARGIN>0  
...
--end                
 ...
 select @MARGINDATE,a.cltcode,0,0,0,0,0,0,0,a.balance,0,0,0,0,0,0,0,0                    
 from                   
 (select a.Cltcode,a.balance from #bb a join (Select party_code from MCDX.dbo.client2 (nolock) where party_code in (select party_code from #aaa)) b  on a.cltcode=b.party_Code ) a                    
 left outer join #NCMS_marh b                   
 on a.cltcode = b.clcode                     
 where b.clcode is null                    
...
(select substring(Cltcode,3,10) as Cltcode,balance as VBal from #bb where cltcode like '98%' and Balance < 0) b                    
 ...
 update #NCMS_marh set UNRecoCr=-b.UnRecoCr from                     
 (select cltcode,SUM(Cramt) as UnRecoCr from BO_client_deposit_recno where cltcode in (select party_code from #aaa) group by cltcode) b                    
 where #NCMS_marh.clcode=b.cltcode                   
...
select @sdtcur=sdtnxt from ACCOUNTMCDX.dbo.parameter where ldtcur        =
(select ldtprv from ACCOUNTMCDX.dbo.parameter where sdtcur <= GETDATE() and ldtcur >=GETDATE()          )
/********************************************************************************/
...
-- and CLTCODE in (select party_code from #aaa)  
 ...
 select @MARGINDATE,a.cltcode,0,0,0,0,0,0,0,a.balance,0,0,0,0,0,0,0,0                    
 from                   
 (select a.Cltcode,a.balance from #bb a join (Select party_code from MCDX.dbo.client2 (nolock)-- where party_code in (select party_code from #aaa)
...
(select substring(Cltcode,3,10) as Cltcode,balance as VBal from #bb where cltcode like '98%' and Balance < 0) b                    
 ...
 where #NCMS_marh.clcode=b.cltcode   
  ...
  /*added for unsetteledDrbills*/
    select cltcode,sum(VAMT) as UnsettledDrBill into #drbills from ACCOUNTMCDX.dbo.ledger WITH(NOLOCK)  
...
Begin         
   select a.* into #NewcollMCX from msajag.dbo.CollateralDetails a WITH (NOLOCK)       
   where exchange='MCX' and segment='FUTURES' and effdate = (select max(effdate) from msajag.dbo.CollateralDetails WITH (NOLOCK)         
...
CONVERT(MONEY, 0) as Value_AHC        
 into #we  from #NewcollMCX a join        
...
DECLARE @MARGINDATE AS DATETIME,@sdtcur as datetime,@NONCASHconsideration as int = 50                
SELECT @MARGINDATE=MAX(MARGINDATE) FROM MCDX.dbo.tbl_clientmargin WITH (NOLOCK)  where MARGINDATE=@FilterDate 
...
CREATE CLUSTERED INDEX IDXCL ON #NCMS_marh(CLCODE)  
 ...
 (                
 Select cltcode,vamt,drcr from ACCOUNTMCDX.dbo.ledger a (nolock)                 
 where VDT>=@FilterDate and VDT <= @FilterDate+' 23:59:59' and VTYP=15) a                
 group by cltcode                
...
AS
    ...
    FROM   accountmcdx.dbo.parameter
    ...
    SELECT vno,
           vtyp
    INTO   #bb
    FROM   accountmcdx.dbo.ledger WITH (nolock)
    ...
    FROM   (SELECT a.vdt,
                   a.cltcode,
                   a.vamt
            FROM   accountmcdx.dbo.ledger a WITH (nolock)
                   INNER JOIN #bb b
                           ON a.vno = b.vno
                              AND a.vtyp = b.vtyp
...
-- Example 
/*
exec [INHOUSE_MCDX].dbo.usp_testing_result @sdate = '2023-04-25',@Category = 'STT'
exec [INHOUSE_MCDX].dbo.usp_testing_result @sdate = '2023-04-25',@Category = 'Stamp Duty'
exec [INHOUSE_MCDX].dbo.usp_testing_result @sdate = '2023-04-28',@Category = 'Day wise Summary - CCD'
exec [INHOUSE_MCDX].dbo.usp_testing_result @sdate = '2023-04-28',@Category = 'CCD'
exec [INHOUSE_MCDX].dbo.usp_testing_result @sdate = '2023-04-28',@Category = 'Day wise Summary - FOSETTLEMENT'
exec [INHOUSE_MCDX].dbo.usp_testing_result @sdate = '2023-04-25',@Category = 'FOSETTLEMENT'
exec [INHOUSE_MCDX].dbo.usp_testing_result @sdate = '2023-04-25',@Category = 'Charges'
exec [INHOUSE_MCDX].dbo.usp_testing_result @sdate = '2023-04-25',@Category = 'BILLVALAN'
*/
-- =============================================
...
BEGIN
	...
	BEGIN
		...
		SUM(PSTTTRD) AS PSTT, SUM(SQTYTRD) AS SQ, SUM(SAMTTRD) AS SAMT, SUM(SSTTTRD) AS SSTT, SUM(TOTALSTT) AS TSTT
		FROM MCDX..STT_ClientDetail WITH (NOLOCK) WHERE SAUDA_DATE = @sdate 
		GROUP BY RECTYPE, INST_TYPE
		) a
	...
	BEGIN
		...
		SUM(BUYSTAMP) AS BUY_ST, SUM(SELLSTAMP) AS SELL_ST, SUM(TOTALSTAMP) AS TOT_ST
		 FROM MCDX..TBL_STAMP_DATA WITH (NOLOCK) WHERE cast(SAUDA_DATE as date)= @sdate --AND PARTY_CODE = 'A1000092'
		--AND TOTALSTAMP <> 0 
		GROUP BY INST_TYPE
		ORDER BY INST_TYPE
	END
	...
	BEGIN
		...
		FROM MSAJAG.dbo.COMMON_CONTRACT_DATA_RTB_FO_MCX with (NOLOCK) WHERE cast(SAUDA_DATE as date)= @sdate -- AND CL_RATE <> 0
		UNION ALL
		SELECT N_TAB = 'ORG', COUNT(DISTINCT PARTY_CODE) CLT, SUM(QTY) AS QTY, SUM(BROKERAGE) AS BROKERAGE, SUM(SERVICE_TAX) AS SERVICE_TAX, 
		SUM(INS_CHRG) AS INS_CHRG, SUM(NETAMOUNT) AS NETAMOUNT , SUM(SEBI_TAX) AS SEBI_TAX, SUM(TURN_TAX) AS TURN_TAX, SUM(BROKER_CHRG) AS BROKER_CHRG, 
		SUM(OTHER_CHRG) AS OTHER_CHRG, SUM(NETAMOUNTALL) AS NETAMOUNTALL, SUM(BROK) AS BROK
		FROM MCDX.dbo.COMMON_CONTRACT_DATA with (NOLOCK) WHERE EXCHANGE = 'MCX' AND SEGMENT = 'FUTURES' AND cast(SAUDA_DATE as date)= @sdate
	END
	...
	BEGIN
		SELECT * FROM (
			...
			SUM(INS_CHRG) AS INS_CHRG, SUM(NETAMOUNT) AS NETAMOUNT , SUM(SEBI_TAX) AS SEBI_TAX, SUM(TURN_TAX) AS TURN_TAX, SUM(BROKER_CHRG) AS BROKER_CHRG, 
			SUM(OTHER_CHRG) AS OTHER_CHRG, SUM(NETAMOUNTALL) AS NETAMOUNTALL, SUM(BROK) AS BROK
			FROM MSAJAG.dbo.COMMON_CONTRACT_DATA_RTB_FO_MCX WITH (NOLOCK) WHERE cast(SAUDA_DATE as date)= @sdate
			...
			SUM(OTHER_CHRG) AS OTHER_CHRG, SUM(NETAMOUNTALL) AS NETAMOUNTALL, SUM(BROK) AS BROK
			 ...
			 EXCHANGE = 'MCX' AND SEGMENT = 'FUTURES' AND cast(SAUDA_DATE as date)= @sdate
			GROUP BY SELL_BUY, BFCF_FLAG
		) A
	...
	BEGIN
		...
		FROM MCDX..FOSETTLEMENT WITH (NOLOCK) WHERE cast(SAUDA_DATE as date)= @sdate
		AND AUCTIONPART = ''
	END
	...
	BEGIN
		SELECT * FROM (
			...
			SUM(TRADE_AMOUNT) AS AMT , SUM(SERVICE_TAX) AS SERVICE_TAX , SUM(SEBI_TAX) AS SEBIFEE, SUM(TURN_TAX) AS TURN_TAX, 
			SUM(BROKER_CHRG) AS SD, SUM(INS_CHRG) AS STT 
			FROM MCDX..FOSETTLEMENT WITH (NOLOCK) WHERE cast(SAUDA_DATE as date)= @sdate -- AND PARTY_CODE = 'D417893'
			AND AUCTIONPART = ''
	...
	BEGIN
		...
		(
			...
			FROM MCDX..CHARGES_DETAIL WITH (NOLOCK) WHERE cast(CD_SAUDA_DATE as date)= @sdate AND CD_AUCTIONPART = ''
	...
	BEGIN
		...
		(
			...
			SUM(PBILLAMT) AS PBILLAMT , SUM(SBILLAMT) AS SBILLAMT, SUM(SEBI_TAX) AS SEBI_TAX, SUM(TURN_TAX) AS TURN_TAX, SUM(BROKER_NOTE) AS SD, SUM(INS_CHRG) AS STT
			 FROM MCDX..FOBILLVALAN WITH (NOLOCK) WHERE cast(SAUDA_DATE as date)= @sdate -- AND PARTY_CODE = 'A1004858'
			 AND AUCTIONPART = ''
			GROUP BY INST_TYPE, TRADETYPE 
		) A
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_Colleteral_27Sep2023
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_Colleteral_27Sep2023]
(
    [party_Code] VARCHAR(10) NULL,
    [CashColl] MONEY NULL,
    [NonCashColl] MONEY NULL
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_01122021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_01122021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_01Oct2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_01Oct2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_02Dec2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_02Dec2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_02Nov2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_02Nov2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_02Sep2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_02Sep2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_03012022
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_03012022]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_03Dec2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_03Dec2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_03Nov2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_03Nov2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_03Sep2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_03Sep2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_04Oct2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_04Oct2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_05012022
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_05012022]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_05Oct2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_05Oct2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_06Dec2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_06Dec2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_06Oct2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_06Oct2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_07122021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_07122021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_07Jan2022
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_07Jan2022]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_07Oct2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_07Oct2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_07Sep2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_07Sep2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_08102021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_08102021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_08Dec2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_08Dec2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_08Nov2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_08Nov2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_08Oct2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_08Oct2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_08Sep2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_08Sep2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_09Dec2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_09Dec2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_09Nov2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_09Nov2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_09Sep2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_09Sep2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_10Jan2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_10Jan2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_10Nov2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_10Nov2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_11082021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_11082021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_11Aug2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_11Aug2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_11Nov2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_11Nov2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_12Oct2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_12Oct2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_13082021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_13082021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_13Aug2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_13Aug2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_13Dec2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_13Dec2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_13Oct2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_13Oct2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_14122021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_14122021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_14Sep2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_14Sep2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_15Dec2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_15Dec2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_15Nov2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_15Nov2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_16Aug2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_16Aug2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_16Dec2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_16Dec2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_16Jul2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_16Jul2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_16Nov2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_16Nov2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_16Sep2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_16Sep2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_17Aug2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_17Aug2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_17Dec2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_17Dec2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_17Nov2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_17Nov2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_17Sep2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_17Sep2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_18112021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_18112021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_18Aug2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_18Aug2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_18Nov2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_18Nov2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_18Oct2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_18Oct2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_19Oct2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_19Oct2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_21Dec2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_21Dec2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_21Oct2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_21Oct2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_21Sep2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_21Sep2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_22112021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_22112021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_22Dec2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_22Dec2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_22Jul2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_22Jul2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_22Oct2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_22Oct2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_22Sep2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_22Sep2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_23122021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_23122021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_23Dec2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_23Dec2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_23Sep2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_23Sep2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_24112021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_24112021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_24Aug2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_24Aug2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_24Dec2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_24Dec2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_24Sep2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_24Sep2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_25112021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_25112021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_25Oct2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_25Oct2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_26Aug2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_26Aug2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_26Oct2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_26Oct2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_27122021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_27122021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_27Aug2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_27Aug2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_27Oct2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_27Oct2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_28Dec2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_28Dec2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_28Oct2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_28Oct2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_28Sep2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_28Sep2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_29Dec2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_29Dec2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_29Jul2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_29Jul2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_29Oct2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_29Oct2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_30112021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_30112021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_30Aug2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_30Aug2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.NCMS_com_31Dec2021
-- --------------------------------------------------
CREATE TABLE [dbo].[NCMS_com_31Dec2021]
(
    [cltcode] VARCHAR(10) NOT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.PRORDERWISEDETAILS_FO_RTB_18Jan2024
-- --------------------------------------------------
CREATE TABLE [dbo].[PRORDERWISEDETAILS_FO_RTB_18Jan2024]
(
    [SrNo] INT IDENTITY(1,1) NOT NULL,
    [Party_Code] VARCHAR(10) NOT NULL,
    [Sauda_Date] DATETIME NOT NULL,
...
-- --------------------------------------------------
-- TABLE dbo.tbl_17Jun2021
-- --------------------------------------------------
CREATE TABLE [dbo].[tbl_17Jun2021]
(
...
-- --------------------------------------------------
-- TABLE dbo.tbl_check_17Jun2021
-- --------------------------------------------------
CREATE TABLE [dbo].[tbl_check_17Jun2021]
(
    [AccountId] NVARCHAR(255) NULL,
    [Amount] FLOAT NULL
);
...
-- --------------------------------------------------
-- TABLE dbo.tbl_clientmarginx
-- --------------------------------------------------
CREATE TABLE [dbo].[tbl_clientmarginx]
(
    [party_code] VARCHAR(15) NOT NULL,
    [margindate] DATETIME NOT NULL,
    [billamount] MONEY NOT NULL,
...
-- --------------------------------------------------
-- TABLE dbo.TBL_PCNORD_RTB21FEB
-- --------------------------------------------------
CREATE TABLE [dbo].[TBL_PCNORD_RTB21FEB]
(
    [party_code] VARCHAR(20) NULL,
    [segment] VARCHAR(20) NULL,
    [order_No] VARCHAR(100) NULL,
...
-- --------------------------------------------------
-- TABLE dbo.TBL_RTB_MCXDATA_UPD
-- --------------------------------------------------
CREATE TABLE [dbo].[TBL_RTB_MCXDATA_UPD]
(
    [SAUDA_DATE] DATETIME NULL,
...
from mcdx.dbo.tblpradnyausers a with (nolock) left outer join mcdx.dbo.tblUserControlMaster b with (nolock)  
on a.fldauto=b.flduserid  
where fldadminauto in (64,65,60,1,9)
...
as  
 ...
 (   
   ...
   where exchange='MCX' and segment='FUTURES' and effdate = (select max(effdate) from anand1.msajag.dbo.CollateralDetails WITH (NOLOCK)   
   where effDate >= getdate()-10 and effDate <= getdate() and exchange='MCX' and segment='FUTURES')    
...
as
select cltcode from accountmcdx.dbo.acmast with (nolock) where       
grpcode in (select grpcode from intranet.roe.dbo.ff_bank_grpcode with (nolock) where segment='MCX')      
and cltcode not in (select cltcode from intranet.roe.dbo.ff_bank_details with (nolock) where segment='MCX' and cltcode <> 0)
...
from accountmcdx.dbo.ledger with (nolock)         
where vdt >= (select sdtcur from accountmcdx.dbo.parameter with (nolock) where sdtcur <=getdate() and ldtcur >=getdate())
and vdt<=getdate()        
