...
-- Server: 10.253.33.93
-- Database: CLASSAPP
-- Exported: 2026-02-05T12:29:23.853834
...
-- --------------------------------------------------
-- PROCEDURE dbo.CLASS_MULTILOGIN_14062017
-- --------------------------------------------------
...
BEGIN
	DECLARE @SQL VARCHAR(MAX),
			@SQL1 VARCHAR(MAX)
	--SET @SQL = 	@SHAREDB + '.DBO.DELTRANS'
   

	SET @SQL = 'SELECT DISTINCT BDPID FROM ' + @SHAREDB + '.DBO.DELTRANS' + ' ORDER BY BDPID ' 
exec(@SQL)
	SET @SQL1 = 'SELECT DISTINCT BCLTDPID FROM ' + @SHAREDB + '.DBO.DELTRANS' + ' ORDER BY BCLTDPID'
exec(@SQL1) 
END
...
BEGIN
	...
	SET @SQL ='SELECT DISTINCT DPID FROM ' + @SHAREDB + '.DBO.DELIVERYDP ORDER BY DPID'
	exec(@SQL)

	SET @SQL1 ='SELECT DISTINCT DPCLTNO FROM ' + @SHAREDB + '.DBO.DELIVERYDP ORDER BY DPCLTNO'
	exec(@SQL1)
END
...
BEGIN
   IF @SETT_TYPE='' AND @SETT_NO=''
		BEGIN
			SELECT DISTINCT SETT_TYPE FROM MSAJAG.DBO.DEMATTRANS ORDER BY SETT_TYPE
		END
	ELSE IF @SETT_TYPE <>'' AND @SETT_NO=''
		BEGIN
			SELECT DISTINCT SETT_NO  
			FROM MSAJAG.DBO.DEMATTRANS 
