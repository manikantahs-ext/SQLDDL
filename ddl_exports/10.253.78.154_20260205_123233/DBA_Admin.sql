...
-- Database: DBA_Admin
-- Exported: 2026-02-05T12:32:33.886550
...
-- --------------------------------------------------
CREATE CLUSTERED INDEX [ci_performance_counters] ON [dbo].[performance_counters] ([collection_time_utc], [host_name])
...
-- --------------------------------------------------
ALTER TABLE [dbo].[BlitzFirst_WaitStats_Categories] ADD CONSTRAINT [PK__BlitzFir__98C9ED57E5985FAD] PRIMARY KEY ([WaitType])
...
BEGIN
	...
	/*
		...
		EXEC dbo.usp_collect_memory_clerks @recipients = 'some_dba_mail_id@gmail.com'
...
-- --------------------------------------------------
-- PROCEDURE dbo.usp_Split_LogBackup
-- --------------------------------------------------
CREATE PROCEDURE [dbo].[usp_Split_LogBackup]

@Database		VARCHAR(100),
@Directory		VARCHAR(500),
@NumberOfFiles	SMALLINT,
@CleanupTime	SMALLINT = 0	-- value of hours
...
/*
	EXEC usp_Split_LogBackup 'Test_LS', '\\10.253.78.154\l$\SQL_Backup\10_253_78_154\NXT$NXT\LogShipping_Split', 16
	SELECT * FROM DBA_Admin.dbo.tbl_Log_Backup_History where database_name = 'NXT' order by backupstart_time
	UPDATE tbl_Log_Backup_History SET IsLogRestored=0, RestoreStartTime=NULL, RestoreEndTime=NULL WHERE BackupStart_Time = '2024-11-29 18:01:39.820'

	DELETE FROM DBA_Admin.dbo.tbl_Log_Backup_History where islogrestored=0
*/

	DECLARE @sSql NVARCHAR(MAX)
	DECLARE @getdate	DATETIME
	DECLARE @fileSuffix	VARCHAR(20)
	DECLARE @BackupFilePath	VARCHAR(500)
	DECLARE @LoopCounter SMALLINT
	DECLARE @FirstBackupFileName	VARCHAR(100)
	DECLARE @LastBackupFilName		VARCHAR(100)

	-- Check if table backup log is present
	IF NOT EXISTS(SELECT * FROM DBA_Admin.sys.tables WHERE name = 'tbl_Log_Backup_History')
	BEGIN
		-- create table
		CREATE TABLE tbl_Log_Backup_History
		(
			Instance_Name		VARCHAR(100),
			Database_Name		VARCHAR(100),
			BackupStart_Time	DATETIME,
			BackupEnd_Time		DATETIME,
			BackupPath			VARCHAR(500),
			Backup_Query		VARCHAR(MAX),
			Restore_Query		VARCHAR(MAX),
			First_BackupFileName	VARCHAR(100),
			Last_BackupFileName		VARCHAR(100),
			IsLogRestored		BIT DEFAULT (0),
	...
	-- Backup file name will be in format of dbName_YYYYMMDDHHMMSS.trn
	SET @getdate = getdate()
	SET @fileSuffix = replace(convert(varchar(10), @getdate, 121), '-', '') + replace(convert(time(0),getDate()),':','')
	SET @LoopCounter = 2

	SET @FirstBackupFileName = @Database + '_' + @fileSuffix + '_01.trn'
	SET @BackupFilePath = @Directory + '\' + @Database + '_' + @fileSuffix + '_01.trn'

	-- Generate backup statement
	SET @sSql = 'BACKUP LOG [' + @Database + '] TO DISK=N''' + @BackupFilePath + ''''

	WHILE @LoopCounter <= @NumberOfFiles
	BEGIN
		SET @BackupFilePath = @Directory + '\' + @Database + '_' + @fileSuffix + '_' + RIGHT('0' + CONVERT(VARCHAR(2), @LoopCounter), 2) + '.trn'

		SET @sSql = @sSql + ', DISK=N''' + @BackupFilePath + ''''

		IF @LoopCounter = @NumberOfFiles
			SET @LastBackupFilName = @Database + '_' + @fileSuffix + '_' + RIGHT('0' + CONVERT(VARCHAR(2), @LoopCounter), 2) + '.trn'
	...
	SET @sSql = @sSql + ' WITH NOFORMAT, NOINIT,  NAME = N''' + @Database + '-Log Database Backup'', SKIP, NOREWIND, NOUNLOAD, COMPRESSION,  STATS = 5'
	PRINT @sSql

	-- Insert entry in tbl_Log_Backup_History
	INSERT INTO DBA_Admin.dbo.tbl_Log_Backup_History (Instance_Name, Database_Name, BackupStart_Time, BackupPath, Backup_Query, First_BackupFileName, Last_BackupFileName)
	VALUES (@@SERVERNAME, @Database, GETDATE(), @Directory, @sSql, @FirstBackupFileName, @LastBackupFilName)

	EXEC sp_executeSql @sSql

	UPDATE DBA_Admin.dbo.tbl_Log_Backup_History SET BackupEnd_Time = GETDATE() WHERE Instance_Name =@@SERVERNAME AND Database_Name = @Database AND BackupEnd_Time IS NULL
...
-- --------------------------------------------------
-- PROCEDURE dbo.usp_Split_LogRestore
-- --------------------------------------------------
CREATE PROCEDURE [dbo].[usp_Split_LogRestore]

@SourceDatabase				VARCHAR(100),
@DestinationDatabase		VARCHAR(100),
@BackupFilePath				VARCHAR(500),
@BackupHistoryTableServer	VARCHAR(50)
...
/*
example:
exec usp_Split_LogRestore 'Test_LS', 'Test_LS', '\\10.253.78.154\l$\SQL_Backup\10_253_78_154\NXT$NXT\LogShipping_Split', '10.253.78.154'

*/

DECLARE @LastRestoredFile	VARCHAR(500)
DECLARE @sSql	NVARCHAR(MAX)

--select TOP 1 Last_BackupFileName from [10.253.78.154].[dba_admin].dbo.tbl_Log_Backup_History WHERE IsLogRestored=1 AND Database_Name = 'Test_LS' AND BackupEnd_Time IS NOT NULL ORDER BY BackupStart_Time DESC

SET @sSql = 'SELECT TOP 1 Last_BackupFileName AS Last_BackupFileName FROM [DBA_Admin].dbo.tbl_Log_Backup_History WHERE IsLogRestored=1 AND Database_Name=''' + @SourceDatabase + ''' AND BackupEnd_Time IS NOT NULL ORDER BY BackupStart_Time DESC'
SET @sSql = 'SELECT Last_BackupFileName FROM OPENQUERY(' + QUOTENAME(@BackupHistoryTableServer) + ',"' + @sSql + '")'
SET @sSql = REPLACE(@sSql, '''', '''''')
SET @sSql = REPLACE(@sSql, '"', '''')

DECLARE @tbl_LastBackupFileName TABLE (Last_BackupFileName VARCHAR(100))
INSERT INTO @tbl_LastBackupFileName EXECUTE sp_ExecuteSQL @sSql

IF EXISTS(SELECT Last_BackupFileName FROM @tbl_LastBackupFileName)
	SELECT @LastRestoredFile = Last_BackupFileName FROM @tbl_LastBackupFileName
ELSE
	SET @LastRestoredFile = '0'
...
--exec xp_dirtree N'\\10.253.78.154\l$\SQL_Backup\10_253_78_154\NXT$NXT\LogShipping_Split', 1, 1

-- backup file in format dbname_YYYYMMDDHHMMSS_nn.trn

--select * from #filelist where subdirectory not like 'Test_LS_[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]!_%' escape '!'
select * from #filelist where subdirectory not like @SourceDatabase + '_[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]!_%' escape '!'


-- delete from #filelist where subdirectory not like 'Test_LS_[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]!_%' escape '!'
delete from #filelist where subdirectory not like @SourceDatabase + '_[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]!_%' escape '!'


;with cte as
(
--select subdirectory, @databasename as databasename, len(@databasename)+1+8+1+4+5 AS PatternLen, left(subdirectory,len(@databasename)+1+8+1+4+6) as pattern   from #filelist
select subdirectory, @SourceDatabase as databasename, len(@SourceDatabase)+1+15 AS PatternLen, left(subdirectory,len(@SourceDatabase)+1+15) as pattern   from #filelist
where subdirectory > @lastrestoredfile
--order by subdirectory
)
,
cte2 as
(
select *, DENSE_RANK() OVER (ORDER BY Pattern) AS BackupID, ROW_NUMBER() OVER(PARTITION BY Pattern ORDER BY Subdirectory) AS RowNumber from cte 
--order by subdirectory
)



select * into #cte_Temp from cte2 
order by subdirectory

select * from #cte_Temp

declare @subdirectory varchar(500), @patternlen int, @pattern varchar(500), @backupid int, @rownumber int
declare @restore_SQL nvarchar(max)

declare cur_restore cursor local forward_only
for 
select distinct  backupid from #cte_Temp 
order by BackupID

open cur_restore
fetch next from cur_restore into  @backupid
while @@FETCH_STATUS = 0
begin
	print 'in loop 1'
	set @restore_SQL = NULL
	set @restore_SQL = 'RESTORE LOG [' + @DestinationDatabase + '] FROM '

	declare cur_files cursor local forward_only for select subdirectory, rownumber from #cte_Temp where BackupID = @backupid
	open cur_files
	fetch next from cur_files into @subdirectory, @rownumber
	while @@FETCH_STATUS = 0
	begin
		
		SET @restore_SQL = @restore_SQL + (CASE WHEN @rownumber > 1 Then ',' ELSE '' END) + ' DISK=''' + @BackupFilePath + '\' + @subdirectory + ''''

		fetch next from cur_files into @subdirectory, @rownumber
	end
	SET @restore_SQL = @restore_SQL + ' WITH STATS = 5, NORECOVERY'


	-- update tbl_Log_Backup_History in parmary server
	SET @sSql = 'SELECT RestoreStartTime FROM [DBA_Admin].dbo.tbl_Log_Backup_History WHERE Last_BackupFileName=''' + @subdirectory + ''''
	SET @sSql = 'UPDATE OPENQUERY(' + QUOTENAME(@BackupHistoryTableServer) + ',"' + @sSql + '") SET RestoreStartTime=GETDATE()'
	SET @sSql = REPLACE(@sSql, '''', '''''')
	SET @sSql = REPLACE(@sSql, '"', '''')
	...
	-- UPDATE Backup Table in primary
	SET @sSql = 'SELECT IsLogRestored, RestoreEndTime FROM [DBA_Admin].dbo.tbl_Log_Backup_History WHERE Last_BackupFileName=''' + @subdirectory + ''''
	SET @sSql = 'UPDATE OPENQUERY(' + QUOTENAME(@BackupHistoryTableServer) + ',"' + @sSql + '") SET IsLogRestored=1, RestoreEndTime=GETDATE()'
	SET @sSql = REPLACE(@sSql, '''', '''''')
	SET @sSql = REPLACE(@sSql, '"', '''')
...
-- --------------------------------------------------
-- TABLE dbo.tbl_Log_Backup_History
-- --------------------------------------------------
CREATE TABLE [dbo].[tbl_Log_Backup_History]
(
    [Instance_Name] VARCHAR(100) NULL,
    [Database_Name] VARCHAR(100) NULL,
    [BackupStart_Time] DATETIME NULL,
    [BackupEnd_Time] DATETIME NULL,
    [BackupPath] VARCHAR(500) NULL,
    [Backup_Query] VARCHAR(MAX) NULL,
    [Restore_Query] VARCHAR(MAX) NULL,
    [First_BackupFileName] VARCHAR(100) NULL,
    [Last_BackupFileName] VARCHAR(100) NULL,
    [IsLogRestored] BIT NULL DEFAULT ((0)),
...
,cte_disk_space_datasource as (select collection_time_utc, host_name, disk_volume, label, capacity_mb, free_mb, block_size, filesystem from [10.253.78.153].[DBA_Admin].dbo.disk_space)
...
,cte_os_tasks_datasource as (select [collection_time_utc], [host_name], [task_name], [pid], [session_name], [memory_kb], [status], [user_name], [cpu_time], [cpu_time_seconds], [window_title] from [10.253.78.153].[DBA_Admin].dbo.os_task_list)
...
,cte_counters_datasource as (select collection_time_utc, host_name, object, counter, value, instance from [10.253.78.153].[DBA_Admin].dbo.performance_counters)
