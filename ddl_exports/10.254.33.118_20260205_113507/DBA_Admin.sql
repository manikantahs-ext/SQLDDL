...
-- Database: DBA_Admin
-- Exported: 2026-02-05T11:35:08.862893
...
-- --------------------------------------------------
-- FUNCTION dbo.fn_get_instanceid
-- --------------------------------------------------
...
-- --------------------------------------------------
ALTER TABLE [dbo].[BlitzFirst_WaitStats_Categories] ADD CONSTRAINT [PK__BlitzFir__98C9ED57D78B346F] PRIMARY KEY ([WaitType])
...
-- --------------------------------------------------
-- PROCEDURE dbo.usp_collect_disk_space
-- --------------------------------------------------
...
-- --------------------------------------------------
-- PROCEDURE dbo.usp_collect_performance_metrics
-- --------------------------------------------------
...
*/
	...
	IF NOT EXISTS(SELECT * FROM [10.253.78.154].DBA_Admin.sys.tables WHERE name = 'tbl_Log_Backup_History')
	BEGIN
		-- create table
	...
	INSERT INTO [10.253.78.154].DBA_Admin.dbo.tbl_Log_Backup_History (Instance_Name, Database_Name, BackupStart_Time, BackupPath, Backup_Query, First_BackupFileName, Last_BackupFileName)
	VALUES (@@SERVERNAME, @Database, GETDATE(), @Directory, @sSql, @FirstBackupFileName, @LastBackupFilName)

	EXEC sp_executeSql @sSql

	UPDATE [10.253.78.154].DBA_Admin.dbo.tbl_Log_Backup_History SET BackupEnd_Time = GETDATE() WHERE Instance_Name =@@SERVERNAME AND Database_Name = @Database AND BackupEnd_Time IS NULL
...
begin
	...
	BEGIN CATCH
		SET @sSql = 'SELECT RestoreStartTime FROM [DBA_Admin].dbo.tbl_Log_Backup_History WHERE Last_BackupFileName=''' + @subdirectory + ''''
		SET @sSql = 'UPDATE OPENQUERY(' + QUOTENAME(@BackupHistoryTableServer) + ',"' + @sSql + '") SET RestoreStartTime=NULL'
		SET @sSql = REPLACE(@sSql, '''', '''''')
		SET @sSql = REPLACE(@sSql, '"', '''')
