...
-- Server: 10.253.50.28
-- Database: CLASSAPP
-- Exported: 2026-01-30T12:38:04.763886
...
SET @SQL = @SQL + " FROM " + @SHAREDB + "..DELTRANS D," + @SHAREDB + "..CLIENT1 C1," + @SHAREDB + "..CLIENT2 C2,"   
SET @SQL = @SQL + " " + @SHAREDB + "..DELIVERYDP DP," + @SHAREDB +"..MFSS_SCRIP_MASTER M"  
...
SET @SQL1 = @SQL1 + " FROM " + @SHAREDB + "..SETT_MST WHERE  SETT_NO = '" + @SETT_NO + "' AND SETT_TYPE = '" + @SETT_TYPE + "')"  
EXEC(@SQL1)  
  
SET @SQL2 = "UPDATE #PAYOUTTOCLIENTSUM SET PAYOUT = (SELECT LEFT(CONVERT(VARCHAR,SEC_PAYOUT,109),11)"   
SET @SQL2 = @SQL2+ " FROM " + @SHAREDB + "..SETT_MST WHERE  SETT_NO = '" + @SETT_NO + "' AND SETT_TYPE = '" + @SETT_TYPE + "')"  
EXEC(@SQL2)  
...
SET @SQL3 = @SQL3 + " BANKNAME = (SELECT BANKNAME FROM " + @SHAREDB + "..BANK WHERE BANKID = DPID)"  
...
-- --------------------------------------------------
-- PROCEDURE dbo.ClassApp_PayoutToClientSum_t
-- --------------------------------------------------
CREATE PROCEDURE [dbo].[ClassApp_PayoutToClientSum_t]  
(  
  @SETT_NO VARCHAR(7),   
  @SETT_TYPE VARCHAR(2),  
  @FROMPARTY VARCHAR(10),  
...
AS  
/*  
EXEC CLASSAPP..GET_SETT_TYPE_NO '1'  
*/  
BEGIN 
 DECLARE @SQL VARCHAR(MAX) 
 IF @FLAG='1'  
  BEGIN  
   SET @SQL='SELECT DISTINCT SETT_TYPE FROM ' + @SHAREDB + '..DELIVERYCLT ORDER BY SETT_TYPE' 
   exec(@SQL) 
  END  
 ELSE  
  BEGIN  
...
/*
		EXEC GETSETTYPE '','',''
		EXEC GETSETTYPE 'A','',''
		EXEC GETSETTYPE 'A','2009180','3'
*/
BEGIN
   IF @SETT_TYPE='' AND @SETT_NO=''
		BEGIN
			SELECT DISTINCT SETT_TYPE FROM NSEMFSS..DEMATTRANS ORDER BY SETT_TYPE
		END
	ELSE IF @SETT_TYPE <>'' AND @SETT_NO=''
		BEGIN
			SELECT DISTINCT SETT_NO  
			FROM NSEMFSS..DEMATTRANS 
	...
	ELSE IF @SETT_TYPE <>'' AND @SETT_NO<>''
		BEGIN
			...
			BEGIN
               IF @PA_PARTY = 'missing'
				BEGIN  
					...
					FROM NSEMFSS..DEMATTRANS 
					WHERE SETT_NO = @SETT_NO AND SETT_TYPE = @SETT_TYPE And PARTY_CODE = 'PARTY' ORDER BY PARTY_CODE
				END
               ELSE
				BEGIN  
					SELECT DISTINCT PARTY_CODE
			...
			BEGIN
                IF @PA_PARTY = 'missing'
				  BEGIN 
					...
					FROM NSEMFSS..DEMATTRANS 
					WHERE SETT_NO = @SETT_NO AND SETT_TYPE = @SETT_TYPE And PARTY_CODE = 'PARTY' ORDER BY SCRIP_CD
				  END 
				ELSE 
				  BEGIN 
					SELECT DISTINCT SCRIP_CD
			...
			BEGIN
				IF @PA_PARTY = 'missing'
				  BEGIN 
					SELECT DISTINCT Left(Convert(Varchar,Trdate,109),11) as TDate
					FROM NSEMFSS..DEMATTRANS 
					WHERE SETT_NO = @SETT_NO AND SETT_TYPE = @SETT_TYPE And PARTY_CODE = 'PARTY' ORDER BY TDate
				  END
				ELSE 
				  BEGIN
					SELECT DISTINCT Left(Convert(Varchar,Trdate,109),11) as TDate
...
*/  
DECLARE @SQL VARCHAR(MAX)
SET @SQL = 	@SHAREDB + '..InsDelAccDebit'

EXEC NSEMFSS..INSDELACCCHECK

insert into Nsemfss..DELACCBALANCE
select distinct party_code, 0, 0 from Nsemfss..DELIVERYCLT
where party_code not in (select cltcode from Nsemfss..DELACCBALANCE)
...
SET @SQL3 = "EXEC " + @SHAREDB + "..INSDELACCCHECK"  
EXEC(@SQL3)  
  
SET @SQL = "insert into " + @SHAREDB + "..DELACCBALANCE"  
SET @SQL = @SQL + " select distinct party_code, 0, 0 from " + @SHAREDB + "..mfss_settlement"  
SET @SQL = @SQL + " where sett_no = '" + @sett_no + "' and sett_Type = '" + @sett_Type + "'"  
SET @SQL = @SQL + " and party_code not in (select cltcode from " + @SHAREDB + "..DELACCBALANCE)"  
EXEC (@SQL)  
...
SET @SQL1 =  @SQL1 + " FROM  " + @SHAREDB + "..CLIENT1 C1," + @SHAREDB + "..CLIENT2 C2," + @SHAREDB + "..DELACCBALANCE A," + @SHAREDB + "..DELTRANS D," + @SHAREDB + "..MFSS_SCRIP_MASTER M"   
...
SET @SQL2 = "UPDATE #DELACCBALANCE SET CL_RATE = NAV_VALUE FROM " + @SHAREDB + "..MFSS_NAV M"  
SET @SQL2 =  @SQL2 + " WHERE NAV_DATE = (SELECT MAX(NAV_DATE) FROM " + @SHAREDB + "..MFSS_NAV M1 WHERE M1.SCRIP_CD = M.SCRIP_CD)"  
...
@StatusId Varchar(15),@StatusName Varchar(25),@HoldDate varchar(11),@SettNo Varchar(7),
@Sett_Type Varchar(10),@Scrip_Cd Varchar(12),@Series varchar(3),@BDpID Varchar(8),@BCltDpID Varchar(16))        
AS       
...
NSEMFSS.DBO.Client1 C1,NSEMFSS.DBO.Client2 C2, NSEMFSS.DBO.MFSS_SCRIP_MASTER M      
...
*/    
IF @SHAREDB ='' SET @SHAREDB='NSEMFSS'
...
IF @SHAREDB = 'NSEMFSS'    
 BEGIN    
  SELECT     
   ...
   DPID      
  FROM     
   NSEMFSS..DELTRANS D, NSEMFSS..MFSS_SCRIP_MASTER M    
  WHERE     
   D.SCRIP_CD = @SCRIP_CD     
   AND D.SERIES= @SERIES     
   AND SETT_NO = @SETT_NO     
...
ELSE IF @SHAREDB = 'BSEMFSS'    
 BEGIN    
  SELECT     
   ...
   DPID      
  FROM     
   BSEMFSS..DELTRANS D, BSEMFSS..MFSS_SCRIP_MASTER M    
  WHERE     
   D.SCRIP_CD = @SCRIP_CD     
   AND D.SERIES= @SERIES     
   AND SETT_NO = @SETT_NO     
...
-- --------------------------------------------------
-- PROCEDURE dbo.sp_SQLRebuildIndex_7
-- --------------------------------------------------

/****** Object:  Stored Procedure dbo.sp_SQLRebuildIndex_7    Script Date: 18/04/2003 8:07:19 PM ******/
CREATE PROCEDURE [sp_SQLRebuildIndex_7] 
	(@table_name nvarchar(384)=null,
	@table_owner nvarchar(384)=null,
	@index_name	sysname = null,
	@file_group_name sysname=null,
	@fill_factor tinyint=null)

AS
/*
Copyright (c) Chong Long Choo

Version 1.0

Release Date: 05 November 2005

Description: This script automate the process of rebuilding one or more indexes for tables in the the current database.

Parameters:
@table_name - table name (optional)
@table_owner - table owner (optional)
@index_name - index name (optional)
@file_group_name - file group that stored an index (optional)
@fill_factor - the percentage of space on each index page to be used for storing data when the index is created.  (optional)

Warning: @fill_factor replaces the original fillfactor of an index. Be carefull when using this parameter

Usage:
exec sp_SQLRebuildIndex [ [ @table_name = ] table_name ],
[ [ table_owner = ] table_owner ],
[ [ @index_name = ] index_name ],
[ [ @file_group_name = ] file_group_name ],
[ [ @fill_factor = ] fill_factor ]

Remarks: This script supports Microsoft SQL Server 2000 and Microsoft SQL Server 7
*/

set nocount on

declare @logical_frag decimal

declare @p1 sysname

set @table_owner=isnull(@table_owner,'%')
set @table_name=isnull(@table_name,'%')
set @file_group_name=isnull(@file_group_name,'%')
set @index_name=isnull(@index_name,'%')

declare objects_to_reindex cursor FORWARD_ONLY READ_ONLY for 
select table_owner = convert(sysname,user_name(o.uid)),
table_name = convert(sysname,o.name),  
index_name = convert(sysname,x.name)
from (sysobjects o
inner join sysindexes x on x.id = o.id) inner join sysfilegroups f on 
f.groupid=x.groupid
where o.type in ('u') and o.name like @table_name
and user_name(o.uid) like @table_owner and
x.name like @index_name and
f.groupname like @file_group_name and (f.status & 0x40 <> 0x40) and
(f.status & 0x8 <> 0x8) and x.indid > 0 and x.indid < 255 and (x.status & 64)=0

open objects_to_reindex

fetch next from objects_to_reindex into @table_owner,@table_name,@index_name

while @@fetch_status=0
begin
	
	set @p1 = @table_owner + '.' + @table_name

	if @fill_factor is null
		dbcc dbreindex(@p1,@index_name) WITH NO_INFOMSGS 
	else
		dbcc dbreindex(@p1,@index_name,@fill_factor) WITH NO_INFOMSGS 

	print 'Rebuilt ' + @index_name + ' in ' + @p1 + ' with fill factor: ' + isnull(cast(@fill_factor as varchar),'Default') 
...
if @@CURSOR_ROWS=0
	print 'There is no index qualify to rebuild'

close objects_to_reindex
deallocate objects_to_reindex
print 'The script is successful!!!'
...
-- --------------------------------------------------
-- PROCEDURE dbo.WRAPPER_Rpt_NseDelHoldDetails
-- --------------------------------------------------

CREATE Proc [dbo].[WRAPPER_Rpt_NseDelHoldDetails] (  
 @StatusId Varchar(15),  
 @StatusName Varchar(25),  
 @HoldDate varchar(11),  
 @SettNo Varchar(7),  
...
AS     
DECLARE @SQL VARCHAR(MAX)  
SET @SQL = @SHAREDB + "..Rpt_NseDelHoldDetails '" + @STATUSID + "', '" + @STATUSNAME + "', '" + @HOLDDATE + "', '" + @SETTNO + "', "  
SET @SQL = @SQL + "'" + @SETT_TYPE + "', '" + @SCRIP_CD + "', '" + @SERIES + "', '" + @BDPID + "', '" + @BCLTDPID + "' "  
...
SET @SQL = 'BSEMFSS.DBO.Rpt_NseDelScripWise_New'
...
-- --------------------------------------------------
-- TABLE dbo.bak_REPORTDETAILS_NEW
-- --------------------------------------------------
CREATE TABLE [dbo].[bak_REPORTDETAILS_NEW]
(
