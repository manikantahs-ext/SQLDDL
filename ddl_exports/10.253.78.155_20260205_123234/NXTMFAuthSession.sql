...
-- Database: NXTMFAuthSession
-- Exported: 2026-02-05T12:32:35.464453

USE NXTMFAuthSession;
GO
...
-- --------------------------------------------------
ALTER TABLE [dbo].[ASPStateTempApplications] ADD CONSTRAINT [PK__ASPState__8E2CF7F9D6D6E45F] PRIMARY KEY ([AppId])
...
-- --------------------------------------------------
ALTER TABLE [dbo].[ASPStateTempSessions] ADD CONSTRAINT [PK__ASPState__C9F49290AEBC5EA9] PRIMARY KEY ([SessionId])
...
-- --------------------------------------------------
        ...
        AS
            CREATE TABLE [NXTMFAuthSession].dbo.ASPStateTempSessions (
            ...
            CREATE NONCLUSTERED INDEX Index_Expires ON [NXTMFAuthSession].dbo.ASPStateTempSessions(Expires)

            CREATE TABLE [NXTMFAuthSession].dbo.ASPStateTempApplications (
                AppId               int             NOT NULL PRIMARY KEY,
                AppName             char(280)       NOT NULL,
            ) 

            CREATE NONCLUSTERED INDEX Index_AppName ON [NXTMFAuthSession].dbo.ASPStateTempApplications(AppName)
...
-- --------------------------------------------------
        ...
        AS
            ...
            INSERT #tblExpiredSessions (SessionId)
                SELECT SessionId
                FROM [NXTMFAuthSession].dbo.ASPStateTempSessions WITH (READUNCOMMITTED)
                WHERE Expires < @now
            ...
            BEGIN 
                ...
                WHILE @@FETCH_STATUS = 0 
                    BEGIN
                        DELETE FROM [NXTMFAuthSession].dbo.ASPStateTempSessions WHERE SessionId = @SessionId AND Expires < @now
                        FETCH NEXT FROM ExpiredSessionCursor INTO @SessionId
                    END
...
-- --------------------------------------------------
    ...
    SELECT @appId = AppId
    FROM [NXTMFAuthSession].dbo.ASPStateTempApplications
    ...
    IF @appId IS NULL BEGIN
        ...
        SELECT @appId = AppId
        FROM [NXTMFAuthSession].dbo.ASPStateTempApplications WITH (TABLOCKX)
        WHERE AppName = @appName
        ...
        BEGIN
            ...
            INSERT [NXTMFAuthSession].dbo.ASPStateTempApplications
            VALUES
            (@appId, @appName)
...
-- --------------------------------------------------
        ...
        AS
            ...
            UPDATE [NXTMFAuthSession].dbo.ASPStateTempSessions
            ...
            IF @length IS NOT NULL BEGIN
                READTEXT [NXTMFAuthSession].dbo.ASPStateTempSessions.SessionItemLong @textptr 0 @length
            END
...
-- --------------------------------------------------
        ...
        AS    
            ...
            INSERT [NXTMFAuthSession].dbo.ASPStateTempSessions 
                (SessionId, 
                 SessionItemLong, 
                 Timeout, 
                 Expires, 
...
-- --------------------------------------------------
    ...
    CREATE PROCEDURE dbo.TempRemoveStateItem
        ...
        @lockCookie int
    AS
        DELETE [NXTMFAuthSession].dbo.ASPStateTempSessions
        WHERE SessionId = @id AND LockCookie = @lockCookie
        RETURN 0
...
-- --------------------------------------------------
-- TABLE dbo.ASPStateTempSessions_BKP_16042018
-- --------------------------------------------------
CREATE TABLE [dbo].[ASPStateTempSessions_BKP_16042018]
(
    [SessionId] NVARCHAR(88) NOT NULL,
