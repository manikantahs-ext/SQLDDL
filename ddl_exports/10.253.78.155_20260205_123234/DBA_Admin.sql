...
-- Database: DBA_Admin
-- Exported: 2026-02-05T12:32:34.999792
...
-- --------------------------------------------------
ALTER TABLE [dbo].[BlitzFirst_WaitStats_Categories] ADD CONSTRAINT [PK__BlitzFir__98C9ED5786980085] PRIMARY KEY ([WaitType])
...
,cte_disk_space_datasource as (select collection_time_utc, host_name, disk_volume, label, capacity_mb, free_mb, block_size, filesystem from [10.253.78.156].[DBA_Admin].dbo.disk_space)
...
,cte_os_tasks_datasource as (select [collection_time_utc], [host_name], [task_name], [pid], [session_name], [memory_kb], [status], [user_name], [cpu_time], [cpu_time_seconds], [window_title] from [10.253.78.156].[DBA_Admin].dbo.os_task_list)
...
,cte_counters_datasource as (select collection_time_utc, host_name, object, counter, value, instance from [10.253.78.156].[DBA_Admin].dbo.performance_counters)
