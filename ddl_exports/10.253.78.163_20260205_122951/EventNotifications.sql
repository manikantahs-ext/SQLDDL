...
-- Exported: 2026-02-05T12:29:56.751237

USE EventNotifications;
GO

-- --------------------------------------------------
-- PK_UNIQUE dbo.DeadLockEvents
-- --------------------------------------------------
ALTER TABLE [dbo].[DeadLockEvents] ADD CONSTRAINT [PK__DeadLock__0378F5030AD2A005] PRIMARY KEY ([EventNumber])
...
-- --------------------------------------------------
-- PROCEDURE dbo.DeadlockEventsProc
-- --------------------------------------------------
Create PROCEDURE [dbo].[DeadlockEventsProc]
AS
SET NOCOUNT ON;
...
BEGIN
   ...
   -- Check to see if the message is an end dialog message.
    ...
    CAST(@message_body.query('(/EVENT_INSTANCE/TextData/deadlock-list)[1]') AS NVARCHAR(MAX))
    ) ;
