-- DDL Export
-- Server: 10.253.33.89
-- Database: CustomizedARQ
-- Exported: 2026-02-05T02:38:40.581610
...
-- --------------------------------------------------
--USP_CUSTARQ_CUST_LIMIT_VALIDATE 'alwr1937'
...
--EXEC USP_CUSTARQ_FILEGENERATION 's167744','25000','100','0','aLTRA','SWSG',''
...
SELECT @COL=@COL+','+ CASE WHEN STRINGVALUE =1 THEN 'LARGE_CAP_STOCKS'  +'*'+  CAST (  (  ( CAST( 25000 AS DECIMAL)/  CAST( @TOTAL AS DECIMAL))*100)/100 AS VARCHAR(100))
					  WHEN STRINGVALUE =2 THEN 'MID_CAP_STOCKS'  +'*'+  CAST (  (  ( CAST( 25000 AS DECIMAL)/  CAST( @TOTAL AS DECIMAL))*100)/100 AS VARCHAR(100))
					  WHEN STRINGVALUE =3 THEN 'MID_CAP_PORTFOLIO'  +'*'+  CAST (  (  ( CAST( 100000 AS DECIMAL)/  CAST( @TOTAL AS DECIMAL))*100)/100 AS VARCHAR(100))
					  WHEN STRINGVALUE =4 THEN 'QUALITY_STOCKS'  +'*'+  CAST (  (  ( CAST( 25000 AS DECIMAL)/  CAST( @TOTAL AS DECIMAL))*100)/100 AS VARCHAR(100))
					  WHEN STRINGVALUE =5 THEN 'VALUE_STOCKS'  +'*'+  CAST (  (  ( CAST( 25000 AS DECIMAL)/  CAST( @TOTAL AS DECIMAL))*100)/100 AS VARCHAR(100))
					  WHEN STRINGVALUE =6 THEN 'FUNDAMENTAL_WINNERS'  +'*'+  CAST (  (  ( CAST( 25000 AS DECIMAL)/  CAST( @TOTAL AS DECIMAL))*100)/100 AS VARCHAR(100))
					  WHEN STRINGVALUE =7 THEN 'STABLE_STOCK_PORTFOLIO'  +'*'+  CAST (  (  ( CAST( 100000 AS DECIMAL)/  CAST( @TOTAL AS DECIMAL))*100)/100 AS VARCHAR(100))
					  END
...
AS BEGIN 
	...
	----GET MINIMUM INVETMENT
		...
		IF(@PROCESS='GET DPID')
		BEGIN
		DECLARE @PanNo NVARCHAR(50) = NULL

		Select @PanNo = pan_gir_no FROM [172.31.16.95].NXT.dbo.sb_client_details WHERE party_Code = @FILTER1

		SELECT client_code as DPID from [172.31.16.108].inhouse.dbo.tbl_client_master with(nolock) 
		where [status] = 'active' and itpan = @PanNo and left(client_Code,8) IN ('12033200','12033201')		
...
-- --------------------------------------------------
-- PROCEDURE dbo.USP_CUSTARQ_GETDATA_NXT
-- --------------------------------------------------
CREATE PROC [dbo].[USP_CUSTARQ_GETDATA_NXT]
@PROCESS AS VARCHAR(100)='',
@FILTER1 AS VARCHAR(5000)='',
@FILTER2 AS VARCHAR(50)='',
@FILTER3 AS VARCHAR(50)='',
...
AS BEGIN 
	 ...
	 SELECT CAST( ROUND( CASE WHEN (@VAL/(CAST (@FILTER2 AS DECIMAL(18,2))/100))%25000=0.0 THEN @VAL/(CAST (@FILTER2 AS DECIMAL(18,2))/100) ELSE (@VAL/(CAST (@FILTER2 AS DECIMAL(18,2))/100))+25000.00-(@VAL/(CAST (@FILTER2 AS DECIMAL(18,2))/100))%25000 END,0)
 AS INT) AS 'MINIMUMINVEST'
			---SELECT CAST( ROUND( CASE WHEN (@VAL%(CAST (@FILTER2 AS DECIMAL(18,2))/100))=0.0 THEN @VAL/(CAST (@FILTER2 AS DECIMAL(18,2))/100) ELSE (@VAL/(CAST (@FILTER2 AS DECIMAL(18,2))/100))+25000.00-@VAL%(CAST (@FILTER2 AS DECIMAL(18,2))/100) END,0) AS INT) AS 'MINIMUMINVEST'
...
--		SELECT ISNULL([ARN No],'') as  ARNNo FROM RISK.DBO.TBL_MF_LS_ARN_EUINS WITH(NOLOCK) WHERE [SB EQUITY TAG]=@ACCESS_CODE
		...
		SELECT ISNULL([ARN No],'') as  ARNNo,
		CASE WHEN [Validity To]<Getdate() tHEN 'ARN Expired' 
		WHEN ISNULL([ARN No],'')='' Then  'ARN Invalid' 
		ELSE 'Valid ARN'
		END Message 
		FROM RISK.DBO.TBL_MF_LS_ARN_EUINS WITH(NOLOCK) WHERE [SB EQUITY TAG]=@ACCESS_CODE
...
-- --------------------------------------------------
--USP_CUSTARQ_GETRECOMMENDATION 'B572','5,6,1','aLTRA'  
...
--DECLARE @RECID VARCHAR(50)='2,4,5,6'  
  ...
  --SELECT CAST( ROUND( CASE WHEN (@VAL%(CAST (@ARQR AS DECIMAL(18,2))/100))=0.0 
  -- THEN @VAL/(CAST (@ARQR AS DECIMAL(18,2))/100) ELSE (@VAL/(CAST (@ARQR AS DECIMAL(18,2))/100))+25000.00-@VAL%(CAST (@ARQR AS DECIMAL(18,2))/100) END,0) AS INT) AS 'MINIMUMINVEST'  
  ...
  ELSE  
  BEGIN  
  SET @MINIMUMINVEST=(SELECT CAST( ROUND( CASE WHEN (@VAL/(CAST (@ARQR AS DECIMAL(18,2))/100))%25000=0.0 THEN @VAL/(CAST (@ARQR AS DECIMAL(18,2))/100) 
  ELSE (@VAL/(CAST (@ARQR AS DECIMAL(18,2))/100))+25000.00-(@VAL/(CAST (@ARQR AS DECIMAL(18,2))/100))%25000 END,0) AS INT))  
  END  
  ...
  SELECT ARQ_TCATEGORY as Category ,ARQ_TSCRIPNAME as Script,V.ISIN as Isin,ARQ_TBENCHMARK as Benckmark,Price = CAST (c.lasttradeprice AS DECIMAL(18,2)),cast(CAST(ROUND(@ARQEQUITYPER/X.NO_STOCK,2) AS decimal(18,2)) as varchar)+'%' AS WEIGHTAGE 
  FROM [ABVSAWUARQ1].ANGEL_WMS.DBO.ARQ_RECO_EQSTOCK_V3 V  
   INNER JOIN [172.31.16.85].odinfeed.dbo.feed_live_eq_bse c WITH (nolock)   
  ON V.arq_tbsescripcode = c.securitycode   
...
-- --------------------------------------------------
-- TABLE dbo.TBL_12STOCK_PROFILE_DATA_23Aug2017
-- --------------------------------------------------
CREATE TABLE [dbo].[TBL_12STOCK_PROFILE_DATA_23Aug2017]
(
    [ID] INT IDENTITY(1,1) NOT NULL,
    [PROFILESID] INT NULL,
    [FUNDID] VARCHAR(50) NULL,
...
-- --------------------------------------------------
-- TABLE dbo.TBL_3STOCK_PROFILE_DATA_23Aug2017
-- --------------------------------------------------
CREATE TABLE [dbo].[TBL_3STOCK_PROFILE_DATA_23Aug2017]
(
    [ID] INT IDENTITY(1,1) NOT NULL,
    [PROFILESID] INT NULL,
    [FUNDID] VARCHAR(50) NULL,
...
-- --------------------------------------------------
-- TABLE dbo.TBL_CUSTARQ_CLIENT_RISKPROFILE2092017
-- --------------------------------------------------
CREATE TABLE [dbo].[TBL_CUSTARQ_CLIENT_RISKPROFILE2092017]
(
    [ID] INT IDENTITY(1,1) NOT NULL,
    [CLIENTCODE] VARCHAR(50) NULL,
    [QNANS] VARCHAR(150) NULL,
