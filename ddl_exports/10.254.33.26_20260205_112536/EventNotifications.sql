...
-- Exported: 2026-02-05T11:25:45.202710
...
-- --------------------------------------------------
-- TRIGGER dbo.Trg_Notify_Changes
-- --------------------------------------------------

-- =============================================  
-- Author: DBA Team  
-- Create date: 24th Dec 2014  
-- Description: Trigger to Notify changes in Client Brokerage Details table  
-- =============================================  
CREATE TRIGGER Trg_Notify_Changes ON LoggedEvents  
AFTER INSERT  
AS  
BEGIN  
 ...
 SELECT @CmdText = TsqlCmdText  
 FROM inserted  
  
 IF (  
   @EventType IN ('CREATE_TRIGGER', 'DROP_TRIGGER', 'ALTER_TRIGGER')  
   AND @DbName = 'MSAJAG'  
   )  
 BEGIN  
  DECLARE @HTMLBODY2 VARCHAR(max)  
  DECLARE @HTMLBODY1 VARCHAR(max)  
  
  SET @HTMLBODY1 = @CmdText  
  SET @HTMLBODY2 = '<HTML><HEAD><STYLE TYPE=''text/css''>' + '.MyTable{width: 850px; border-right: black 1px solid; border-left: black 1px solid; border-bottom: black 1px solid; border-top: black 1px solid; }' + '.MyTable td{width: auto; border-right: bla
ck 1px solid; border-top: black 1px solid; font-size: 0.8em; vertical-align: middle; border-left: black 1px solid; color: black; border-bottom: black 1px solid; font-family: Verdana;background-color: White; text-align: left; height: 18px;}' + '.MyTable ca
ption{font-size: 0.9em; color: White; font-style: normal; font-family: Verdana; background-color: #3c6b97; border-right: black 1px solid; border-top: black 1px solid; border-left: black 1px solid;}' + '.MyTable th{background-color: #95B3D7;border-style : 
solid;border-color : Black; border-width:1px;}' + '</STYLE></HEAD><Body><br>' + 'Dear Team,<BR><BR> &nbsp&nbsp Kindly check the impact due to Trigger Changes.' + '<BR><BR>' + '<center><Div><center>' + @HTMLBODY1 + '</center></Div></center> ' +   
   '<BR><BR>' + '<BR><BR> This is a system-generated e-mail, please do not reply to this email.' + '<BR><BR>Thanks and Regards,<BR> DBA Team <BR></Body></HTML>'  
  
  EXEC msdb.dbo.sp_send_dbmail @profile_name = 'DBA', @recipients = 'inhouse.support@angelbroking.com;Mindgate.Support@angelbroking.com', @subject = 'Trigger Changes on AngelNseCM', @body = @HTMLBODY2, @body_format = 'HTML'  
 END  
END
